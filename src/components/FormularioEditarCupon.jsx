...
function FormularioEditarCupon() {
    ...
    const obtenerDatosServidor = async () => {           
        ...
        try {                        
            ...
            const response = await fetch(`${urlBaseApi}/api/cupon/${id}`, opciones);
            const datos = await response.json();   
            if (response.ok){      
                setCodigo(datos.clave);
                setCompraMinima(Math.floor(datos.compra_minima));
                setMaximoRedimir(datos.maximo_a_redimir);
                setTipo(datos.tipo);
                setValor(Math.floor(datos.valor));
                setEstado(datos.estado_cupon);

                const partes_inicio = datos.fecha_inicio.split(" ");
                const partes_inicio2 = partes_inicio[0].split("-");                                                
                setFechaInicio(new Date(partes_inicio2[0], partes_inicio2[1]-1, partes_inicio2[2]));
                const partes_inicio_hora = partes_inicio[1].split(":");
                setHoraInicio(partes_inicio_hora[0]);
                setMinutoInicio(partes_inicio_hora[1]);

                const partes_fin = datos.fecha_fin.split(" ");
                const partes_fin2 = partes_fin[0].split("-");
                setFechaFin(new Date(partes_fin2[0], partes_fin2[1]-1, partes_fin2[2]));
                const partes_fin_hora = partes_fin[1].split(":");
                setHoraFin(partes_fin_hora[0]);
                setMinutoFin(partes_fin_hora[1]);
    ...
    const handleEditarCupon = async (event) => {
        event.preventDefault();
        reiniciarErrorCampoGlobal();
                
        const raw = {           
            'clave': codigo,            
            'fecha_inicio': format(fechaInicio, 'yyyy-MM-dd')+' '+horaInicio+':'+minutoInicio+':00',            
            'fecha_fin': format(fechaFin, 'yyyy-MM-dd')+' '+horaFin+':'+minutoFin+':00',
            'compra_minima':compraMinima.toString(),
            'maximo_a_redimir': maximoRedimir.toString(),
            'tipo': tipo,
        ...
        try {
            ...
            const datos = await response.json();                        
            if (response.ok){                
                setPopup({mostrar:true, titulo:'Listo', contenido:'Cupón editado.'});
                return;
            } else {
        ...
        <div className="dashboard-content-wrap">
            <div className="container-fluid">
                <div className="dashboard-heading mb-5">                    
                    <h3 className="fs-22 font-weight-semi-bold"><Link to={`/cupon`}><div className="icon-element icon-element-sm shadow-sm cursor-pointer ml-1 text-secondary" data-toggle="tooltip" data-placement="top" data-title="Volver a la lista de cupones"><i className="la la-angle-left"></i></div></Link>&nbsp;Editar cupón</h3>                    
                </div>
                <form action="#">                      
                    ...
                    <div className="course-submit-btn-box pb-4">
                        <button className="btn theme-btn" type="submit" onClick={handleEditarCupon}>Guardar cambios</button>
                    </div>
                </form>
            </div>
        </div>
...
export default FormularioEditarCupon;