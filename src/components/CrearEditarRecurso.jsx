...
export default function CrearEditarRecurso({funcionMostrarPopUp, id_curso, id_categoria, id_recurso=-1}) {
    ...
    const [popUpResource, setPopupResource] = useState({mostrar:true, nombre:'', descripcion:'', archivo:'', archivo_vista_previa:'', id_curso:-1, id_categoria: -1});
    ...
    const handleResource = {
        ...
        save          : async (event) => {
            ...
            if(id_recurso != -1){
                ...
                if(file){
                    const response = await fetch(`${urlBaseApi}/api/recurso/actualizarArchivo/${id_recurso}`, opcionesArchivo);                    
                    if (!response.ok){                                            
                        setMostrarSpinner(false);
                        const datos = await response.json();
                ...
                if(preview){
                    const responsePreview = await fetch(`${urlBaseApi}/api/recurso/actualizarImagenVistaPrevia/${id_recurso}`, opcionesPreview);                    
                ...
                const responseRaw = await fetch(`${urlBaseApi}/api/recurso/${id_recurso}`, opcionesData);
                if (responseRaw.ok){                                            
                    setPopup({mostrar:true, titulo:'Listo', contenido:'Recurso guardado.', data_switch:'cerrar_ventana'});
                } else {
                ...
                setMostrarSpinner(false);
                if (response.ok){                                            
                    setPopup({mostrar:true, titulo:'Listo', contenido:'Recurso creado.', data_switch:'cerrar_ventana'});
                    return;
                } else {
                    const datos = await response.json();
                    mensajesDeError(setPopup, response.status, (typeof datos.datos !== 'undefined') ? datos.datos : {}, setErrorCampoGlobal, {'titulo': '', 'contenido': ''});
    ...
    return (
        ...
        <Popup 
            ...
            textoCerrar="Aceptar"
        />
        <div className="modal fade modal-container show" style={{ background: 'rgba(0, 0, 0, 0.7)' }} id="resourceModal" tabIndex="-1" role="dialog" aria-labelledby="resourceModalTitle" aria-hidden="true">
            <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div className="modal-content">
                    <div className="modal-header border-bottom-gray">
                        <div className="pr-2">                            
                            <h5 className="modal-title fs-19 font-weight-semi-bold lh-24" id="resourceModalTitle">{id_recurso!=-1 ? 'Editar recurso' : 'Crear recurso' }</h5>
                        </div>                            
                    </div>
                    <div className="modal-body">
                        ...
                        <div className="col-lg-12">
                            <div className="form-group">
                                ...
                                <input type="file" name="resourceArchivo" className="form-control form--control user-text-editor pl-3"></input>
                    ...
                    <div className="modal-footer border-top-gray">
                        <button type="button" className="btn theme-btn mb-2" onClick={handleResource.save} >{id_recurso != -1 ? 'Guardar' : 'Crear'}</button>                             
                        <button type="button" className="btn theme-btn theme-btn-white mb-2" onClick={handleResource.close}> Cancelar </button>
                    </div>
                </div>
            </div>
        </div>        
