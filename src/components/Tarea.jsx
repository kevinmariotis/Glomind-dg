...
export default function Tarea({ id_contenido, id_curso, es_docente }) {
  ...
  const [popUpObjeto, setPopupObjeto] = useState({
    mostrar: true,
    id: -1,
    consumo_estado: 0,
    consumo_puntuacion: -1,
    nombre: "",
    descripcion: "",
    fecha_hora_inicio: "",
    fecha_hora_fin: "",
  ...
  const handleEntrega = {
    calificacion: (event) => {
      setEntregaViendo({ ...entregaViendo, calificacion: event.target.value });
    },
    retroalimentacion: (event) => {
      setEntregaViendo({
        ...entregaViendo,
    ...
    get: async (event) => {
      ...
      try {
        ...
        const response = await fetch(
          `${urlBaseApi}/api/tareaentrega/${idEntregaViendo}`,
          opciones
        );
        ...
        if (response.ok) {
          ...
          datos.retroalimentacion =
            datos.retroalimentacion == null ? "" : datos.retroalimentacion;
    ...
    create: async (event) => {
      ...
      if (response.ok) {
        setPopup({
          mostrar: true,
          titulo: "Listo",
          contenido: "Su tarea fue entregada.",
        });
        handleObjeto.getEnvios();
        return;
      } else {
    ...
    save: async (event) => {
      ...
      if (responseRaw.ok) {
        ...
        setPopup({
          mostrar: true,
          titulo: "Listo",
          contenido: "Entrega calificada.",
          data_switch: "volver_a_entregas",
        });
      } else {
    ...
    delete: async (id_tarea_entrega) => {
      ...
      try {
        ...
        const response = await fetch(
          `${urlBaseApi}/api/tareaentrega/${id_tarea_entrega}/${id_curso}`,
          opciones
        );
        //setMostrarSpinner(false);
        const datos = await response.json();
  ...
  const handleObjeto = {
    ...
    getEnvios: async (id_tarea_obtener = -1) => {
      ...
      try {
        ...
        //setMostrarSpinner(true);
        const response = await fetch(
          `${urlBaseApi}/api/tarea/getEnvios/${
            id_tarea_obtener != -1 ? id_tarea_obtener : idTarea
          }/${paginaNavegacion}/tarea_entrega.fecha_hora_entrega-asc/filter_by_id_tarea=0`,
          opciones
        );
        //setMostrarSpinner(false);
        if (response.ok) {
  ...
  const handleEliminarEntrega = (id_tarea_entrega) => {
    setPopup({
      ...
      tipo: 3,
      titulo: "Confirmar?",
      contenido: "Confirma que desea invalidar la entrega?",
      data_switch: "eliminar_tarea_entrega",
      data_id: id_tarea_entrega,
      data_id_2: -1,
    });
  };
  ...
  const handleItemClick = async (item) => {
    ...
    link.href = blobUrl;
    link.download = `tarea_${item.id_tarea}_${item.nombres} ${
      item.apellidos
    }.${item.ruta_archivo.split(".").pop()}`;
  ...
  const mentajesNoEnvio = {
    "fuera-de-tiempo":
      "No es posible enviar la tarea por estar por fuera del rango de tiempo establecido.",
      ...
      {popUpObjeto.id !== -1 ? (
        <>
          <div
            ...
            style={{ background: "var(--Lavander)", color: "#fff" }}
          >
            ...
            <div className="container">
              <div className="breadcrumb-content pt-40px ">
                <div className="section-heading">
                  <h2
                    className="section__title fs-30 pb-2"
                    style={{ color: "#fff" }}
                  >
                    <i className="la la-pencil-square-o mr-2"></i>
                    {popUpObjeto.nombre}
                  </h2>
          ...
          {idEntregaViendo == -1 ? (
            <>
              <div className="bg-light pt-60px pb-60px">
                <div className="container">
                  <div className="row">
                    <div className="col-lg-4 responsive-column-half">
                      <div className="quiz-result-content text-center">
                        ...
                        <h2 className="section__title">{totalEntregas}</h2>
                      </div>
                    </div>
                    <div className="col-lg-4 responsive-column-half">
                      <div className="quiz-result-content text-center">
                            ...
                            <tbody>
                              {Object.keys(entregas).map((key) => (
                                <tr key={`entrega-${entregas[key].id}`}>
                                    ...
                                    <div className="media media-card  pb-2 mb-2">
                                      <div className="media-img mr-4 rounded-full">
                                        <img
                                          ...
                                          src={
                                            entregas[key].imagen_pequena == null
                                              ? `${urlBase}/images/avatar_docente.jpg`
                                              : `${urlBaseApi}/${entregas[key].imagen_pequena}`
                                          }
                                  ...
                                  <td>
                                    {entregas[key].nombres}{" "}
                                    {entregas[key].apellidos}
                                  </td>
                                  <td>{entregas[key].fecha_hora_entrega}</td>
                                  <td>
                                    {entregas[key].estado != 2
                                      ? estadoTarea[entregas[key].estado]
                                      : entregas[key].calificacion}
                                  </td>
                                  <td>
                                    {es_docente ? (
                                      entregas[key].estado == 1 ? (
                                        <button
                                          key={`calificar-${entregas[key].id}`}
                                      ...
                                      <button
                                        key={`descargable-${entregas[key].id}`}
                                        className="btn theme-btn btn-round"
                                        onClick={(event) => {
                                          event.preventDefault();
                                          handleItemClick(entregas[key]);
                                        }}
                                      >
                ...
                <div className="container">
                  <div className="col-lg-6">
                    <div className="form-group">
                      ...
                      <br />
                      <button
                        key={`descargable-x-${entregaViendo.id}`}
                  ...
                  <div className="col-lg-6">
                    <div className="form-group">
                      ...
                      <textarea
                        value={entregaViendo.retroalimentacion}
                        onChange={handleEntrega.retroalimentacion}
                        className="form-control form--control user-text-editor pl-3"
                        name="retroalimentacion"
                      ></textarea>
                      {erroresCampos["retroalimentacion"].length > 0 && (
                        <SpamError
                          mensaje={erroresCampos["retroalimentacion"]}
                        />
                      )}
                    </div>
                  </div>
                  <div className="col-lg-6">
                    <div className="form-group">
                      ...
                      <input
                        value={entregaViendo.calificacion}
                        onChange={handleEntrega.calificacion}
                  ...
                  <div className="modal-footer ">
                    <button
                      ...
                      onClick={handleEntrega.save}
                    >
                      {id_contenido != -1 ? "Guardar" : "Crear"}
                    </button>
                    <button
                      type="button"
                      className="btn theme-btn theme-btn-white mb-2"
                      onClick={handleEntrega.close}
                    >
                      {" "}
                      Volver{" "}
                    </button>
          ...
          {!es_docente ? (
            <>
              <div className="pt-60px pb-60px">
                <div className="container">
                  <div className="breadcrumb-content pt-40px text-start">
                    <div className="section-heading">
                          ...
                          {popUpObjeto.permitir_enviar == "si" ? (
                            <>
                              <label className="label-text">
                                <p className="section__desc pb-2">
                                  Subir entrega
                                </p>
                              </label>
                              <input
                            ...
                            <label className="label-text w-100">
                              <p className="section__desc pb-2 text-center">
                                {mentajesNoEnvio[popUpObjeto.permitir_enviar]}
                              </p>
                            </label>
                          )}
                        </div>
                      ...
                      {popUpObjeto.permitir_enviar == "si" && (
                        <div className="col-lg-12">
                          <button
                            type="button"
                            className="btn theme-btn mb-2 btn-round"
                            onClick={handleEntrega.create}
                          >
                            Enviar archivo
                          </button>
                        </div>
