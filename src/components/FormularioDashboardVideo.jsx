...
function FormularioDashboardVideo() {
  ...
  const handleBorrar = (id_video) => {
    setPopup({
      ...
      tipo: 3,
      titulo: "Confirmar?",
      contenido: "Confirma que desea borrar el video?",
      data_switch: "eliminar_video",
      data_id: id_video,
      data_id_2: -1,
    });
  };
  ...
  const obtenerDatosVideos = async () => {
    ...
    try {
      ...
      const response2 = await fetch(
        `${urlBaseApi}/api/video/getTodos/${paginaNavegacion}/created_at-desc/${palabraBuscar}`,
        opciones
      );
        ...
        <div className="container-fluid">
          <div className="breadcrumb-content d-flex flex-wrap align-items-center justify-content-between mb-5">
            ...
            <div className="btn-box pt-30px">
              {permissions[27] && (
                <Link to="/video/crear" className="btn theme-btn">
                  <i className="la la-plus mr-2"></i> Subir nuevo video
                </Link>
              )}
            </div>
          </div>
            ...
            <div className="col-lg-6">
              <div className="form-group">
                ...
                <input
                  ...
                  name="buscar_video"
                  maxLength="32"
                  placeholder="Ej: Leccion 1, Manipulación del DOM"
                />
              </div>
            </div>
          </div>
