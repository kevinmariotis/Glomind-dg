...
function FormularioEditarVideo() {
  ...
  const { jwt, permissions, urlAmigableVolver, temaActual } =
  ...
  const [popUpSubida, setPopupSubida] = useState({
    mostrar: false,
    titulo: "",
    contenido: "",
  });
  ...
  const [rangoSeleccionadoConvertido, setRangoSeleccionadoConvertido] =
    useState("No cambiar la actual");
  const [imagenMiniaturaSeleccionada, setImagenMiniaturaSeleccionada] =
    useState(`${urlBase}/images/course-no-image.png`);
  const [mostrarVentanaCrearSegmento, setMostrarVentanaCrearSegmento] =
    useState(false);
  const [tieneSegmentosDesactivados, setTieneSegmentosDesactivados] =
    useState(false);
  ...
  const [
    rangoSeleccionadoConvertidoSegmento,
    setRangoSeleccionadoConvertidoSegmento,
  ] = useState(0);
  ...
  const onDrop = (acceptedFiles) => {
    ...
    setRangoSeleccionadoConvertido(
      acceptedFiles[0] == null
        ? rangoSeleccionadoConvertido
        : "No cambiar la actual"
    );
    setImagenMiniaturaSeleccionada(
      acceptedFiles[0] == null
        ? imagenMiniaturaSeleccionada
        : `${urlBase}/images/course-no-image.png`
    );
  };
    ...
    if (selectedVideo != null) {
        ...
        if (status >= 200 && status < 300) {
          ...
          mensajesDeError(
            ...
            setErrorCampoGlobal,
            {
              titulo: "Error",
              contenido:
                "Hubo un error al subir el video, revise el formulario.",
            }
          );
        }
        return;
      ...
      xhr.onerror = () => {
        ...
        mensajesDeError(
          ...
          {
            titulo: "Error",
            contenido: "Hubo un error al subir el video, revise el formulario.",
          }
        );
        return;
      };
        ...
        if (response.ok) {
          if (rangoSeleccionado != -1) {
            ...
            setPopup({
              mostrar: true,
              titulo: "Listo",
              contenido: "Video actualizado satisfactoriamente",
            });
            obtenerDatosServidor();
            return;
          }
    ...
    if (rangoSeleccionado != -1) {
      ...
      try {
        const response = await fetch(
          `${urlBaseApi}/api/video/generarVistaPrevia/${id}/${rangoSeleccionado}`,
          opciones
        );
        const datos = await response.json();
        if (response.ok) {
  ...
  const handleRangoSeleccionado = (event) => {
    ...
    if (event.target.value != -1) {
      const duracion_hms = convertirSegundosAHorasMinutosSegundos(
        event.target.value
      );
      setRangoSeleccionadoConvertido(
        duracion_hms.horas +
          ":" +
          duracion_hms.minutos +
          ":" +
          duracion_hms.segundos
      );
    } else {
  ...
  const handleRangoSoltado = async (event) => {
    ...
    if (event.target.value != -1) {
      ...
      try {
        ...
        const response = await fetch(
          `${urlBaseApi}/api/video/generarVistaPrevia/${id}/${event.target.value}/true`,
          opciones
        );
  ...
  const handleFuncionCerrarPopUpCrearSegmento = () => {
  ...
  const handleNombreSegmentoChange = (event) => {
    setNombreSegmento(event.target.value);
  };
  ...
  const handleEditarSegmento = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/videosegmento/${editarMarcador}`,
        opciones
      );
      setMostrarSpinner(false);
      const datos = await response.json();
  ...
  const handleBorrarMarcador = async (id_video_segmento) => {
    ...
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/videosegmento/${id_video_segmento}`,
        opciones
      );
      setMostrarSpinner(false);
      const datos = await response.blob();
      if (response.ok) {
        setPopup({
          mostrar: true,
          titulo: "Listo",
          contenido: "Marcador borrado.",
        });
        setSegmentos((prevData) =>
          prevData.filter((item) => item.id !== id_video_segmento)
        );
      } else {
        mensajesDeError(
          setPopup,
  ...
  return (
    <>
          ...
          <video
            ...
            poster={`${
              videoImagenVistaPrevia != ""
                ? `${urlBaseApi}/${videoImagenVistaPrevia}`
      ...
      <Modal
        show={mostrarVentanaCrearSegmento}
        size="lg"
        onHide={handleFuncionCerrarPopUpCrearSegmento}
        backdrop="static"
        keyboard={true}
        animation={false}
        centered
      >
        <Modal.Header>
          <Modal.Title>
            {editarMarcador == 0 ? "Crear Marcador" : "Editar Marcador"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="row">
            ...
            <div className="col-lg-12">
              <div className="form-group">
                <label className="label-text">
                  Seleccione el minuto/segundo de inicio del marcador
                </label>
                ...
                <input
                  ...
                  value={rangoSeleccionadoSegmento}
                  onChange={handleRangoSeleccionadoSegmento}
                  className="form-range"
                  id="customRange1"
                  style={{ width: "100%" }}
                ></input>
                {erroresCampos["segundo_inicio"].length > 0 && (
                  <SpamError mensaje={erroresCampos["segundo_inicio"]} />
                )}
              </div>
            </div>
          </div>
        ...
        <Modal.Footer>
          <Button variant="primary" onClick={handleCrearSegmento}>
            Guardar
          </Button>
          <Button
            variant="secondary"
            onClick={handleFuncionCerrarPopUpCrearSegmento}
          >
            Cancelar
          </Button>
        </Modal.Footer>
        ...
        <div className="container-fluid">
          <div className="dashboard-heading mb-5">
            <h3 className="fs-22 font-weight-semi-bold">
              <Link to={id_curso ? `/curso/contenido/${id_curso}` : `/video`}>
                <div
                  className="icon-element icon-element-sm shadow-sm cursor-pointer ml-1 text-secondary"
                  data-toggle="tooltip"
                  data-placement="top"
                  data-title="Volver a la lista de videos"
                >
          ...
          <form action="#">
            {permissions[28] || urlAmigableVolver != "" ? (
              <div className="card card-item">
                <div className="card-body">
                  ...
                  <div className="row">
                    <div className="col-lg-12">
                      <div className="form-group">
                        ...
                        <input
                          ...
                          maxLength="128"
                          value={nombre}
                          placeholder="Ej: Lección 1: Manipulación del dom"
                        />
                        {erroresCampos["nombre"].length > 0 && (
                          <SpamError mensaje={erroresCampos["nombre"]} />
                        )}
                    ...
                    <div className="col-lg-12">
                      <div className="form-group">
                        <label className="label-text">Tipo de video</label>
                        <select
            ...
            {permissions[71] || urlAmigableVolver != "" ? (
              <div className="card card-item">
                <div className="card-body">
                  ...
                  <div className="row">
                    <div className="col-lg-12">
                      <div className="form-group mb-0">
                          ...
                          <div className="course-item-content-wrap">
                            <div className="form-group media media-card">
                              ...
                              >
                                ...
                                videoImagenVistaPrevia != "" ? (
                                  <img
                                    src={`${urlBaseApi}/${videoImagenVistaPrevia}`}
                                    alt={nombre}
                                    onClick={() => {
                                      setPopupVistaPrevia({
                                        ...popUpVistaPrevia,
                                        mostrar: true,
                                        contenido: videoActual,
                                      });
                                    }}
                                  />
                                ) : (
                        ...
                        <ul>{fileList}</ul>
                        {erroresCampos["video_grande"].length > 0 && (
                          <SpamError mensaje={erroresCampos["video_grande"]} />
                        )}
                      </div>
                    </div>
                  </div>
            ...
            {permissions[70] || urlAmigableVolver != "" ? (
              <div className="card card-item">
                <div className="card-body">
                  <h3 className="fs-22 font-weight-semi-bold pb-2">
                    Marcadores
                  ...
                  <div className="row">
                    <div className="col-lg-12">
                      <div className="form-group">
                        {tieneSegmentosDesactivados && (
                          <span className="badge badge-warning">
                            Los marcadores se desactivaron debido a que el video
                            fue reemplazado por otro y posiblemente los
                            marcadores no concidan, por favor revisar los
                            marcadores.
                          </span>
                        )}
                          ...
                          <table className="table generic-table">
                            ...
                            <tbody>
                              {Object.keys(segmentos).map((key) => (
                                <tr
                                  key={`segmento-seleccion-${segmentos[key].id}`}
                                >
                                  <td>{segmentos[key].segundo_inicio_hms}</td>
                                  <td>{segmentos[key].nombre}</td>
                                  <td>
                                    {segmentos[key].estado == 1 ? (
                                      "Activado"
                                    ) : (
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => {
                                        handleEditarMarcador(
                                          segmentos[key].id,
                                          segmentos[key].nombre,
                                          segmentos[key].segundo_inicio
                                        );
                                      }}
                                  ...
                                  <td>
                                    <button
                                      type="button"
                                      onClick={() => {
                                        handleBorrarMarcador(segmentos[key].id);
                                      }}
                                      className="icon-element icon-element-xs shadow-sm border-0"
                                      data-toggle="tooltip"
                                      data-placement="top"
                        ...
                        <div className="course-submit-btn-box pb-4">
                          <button
                            ...
                            type="submit"
                            onClick={(event) => {
                              handleAgregarMarcador(event);
                            }}
                          >
                            <i className="la la-plus mr-2"></i>Agregar marcador
                          </button>
                        </div>
                      </div>
                    </div>
            ...
            {permissions[72] || urlAmigableVolver != "" ? (
              <div className="card card-item">
                <div className="card-body">
                  <h3 className="fs-22 font-weight-semi-bold pb-2">
                    Imagen miniatura
                      ...
                      <div className="form-group mb-0">
                        ...
                        <div className="course-item-content-wrap">
                          <div className="form-group media media-card">
                            ...
                            >
                              <img
                                src={`${imagenMiniaturaSeleccionada}`}
                                alt={nombre}
                              />
                            </div>
                          </div>
                        ...
                        <input
                          disabled={deshabilitarRange}
                          type="range"
                          min="-1"
                          max={duracionSegundos - 1}
                          value={rangoSeleccionado}
                          onChange={handleRangoSeleccionado}
                          onMouseUp={handleRangoSoltado}
                          onTouchEnd={handleRangoSoltado}
                          onKeyUp={handleRangoSoltado}
                          className="form-range"
                          id="customRange1"
                          style={{ width: "50%" }}
                        ></input>
