...
export default function FormularioEstudianteDetalles() {
    ...
    const [datos, setDatos] = useState({nombres:'', apellidos:'', es_docente:0, cantidad_cursos:0, cantidad_reviews:0, cantidad_comentarios:0, docente_descripcion:null,  imagen_pequena:null, miembro_desde:'', profesion:'', facebook:null, twitter:null, instagram:null, linkedin:null, youtube:null});
    const [cursos, setCursos] = useState({});
    const [totalCursos, setTotalCursos] = useState(0);
    const [paginaCursos, setPaginaCursos] = useState(1);    

    const [cursosFavoritos, setCursosFavoritos] = useState({});
    const [totalCursosFavoritos, setTotalCursosFavoritos] = useState(0);
    const [paginaCursosFavoritos, setPaginaCursosFavoritos] = useState(1);    

    const [cursosInstructor, setCursosInstructor] = useState({});
    const [totalCursosInstructor, setTotalCursosInstructor] = useState(0);
    const [paginaCursosInstructor, setPaginaCursosInstructor] = useState(1);    
    ...
    useEffect(() => {                       
        switch(pestanaActivada){
            case 1:
                obtenerAprendizaje();
                obtenerCursosInstructor();
            break;
            case 2:
                obtenerFavoritos();
            break;            
        }        
    }, [pestanaActivada, paginaCursos, paginaCursosFavoritos]);
    ...
    const obtenerAprendizaje = async () => {
        console.log("refer_rul", refer_url);
        const headers = {
        ...
        try {               
            ...
            const response = await fetch(`${urlBaseApi}/api/usuario/presentacion/${id}/${paginaCursos}`, opciones);
            setMostrarSpinner(false);
            if (response.ok){                           
                const datos = await response.json();   
                //setCategoriaaSistema(datos);
                setCursos(datos.usuario.cursos);
                datos.usuario.cursos = null;                
                setDatos(datos.usuario);                                
                setTotalCursos(datos.usuario.total_cursos);
    ...
    const obtenerFavoritos = async () => {
        ...
        try {               
            ...
            //setMostrarSpinner(true);
            const response = await fetch(`${urlBaseApi}/api/usuario/getFavoritos/${id}/${paginaCursosFavoritos}/1/nombre-asc/6`, opciones);
            //setMostrarSpinner(false);
            if (response.ok){                           
                const datos = await response.json();                  
                setCursosFavoritos(datos.cursos);                
                setTotalCursosFavoritos(datos.total_cursos);
            } else {      
                const data = await response.json();          
                mensajesDeError(setPopup, response.status, (typeof data.datos !== 'undefined') ? data.datos : {});                
            }
    ...
    const obtenerCursosInstructor = async () => {
        const headers = {
            'Authorization':`Bearer ${jwt}`,
        }        
        try {               
            ...
            //setMostrarSpinner(true);
            const response = await fetch(`${urlBaseApi}/api/usuario/getCursosInstructor/${id}/${paginaCursosInstructor}/6/visibles/nombre-desc`, opciones);
            //setMostrarSpinner(false);
            if (response.ok){                           
                const datos = await response.json();                  
                setCursosInstructor(datos.cursos);                
                setTotalCursosInstructor(datos.total_cursos);
            } else {      
                const data = await response.json();          
                mensajesDeError(setPopup, response.status, (typeof data.datos !== 'undefined') ? data.datos : {});                
            }
    ...
    return (
        <>
            ...
            {refer_url!=undefined && <section className="breadcrumb-area">
                <div className="bg-white py-3 pattern-bg" style={{zIndex:'0'}}>
                    <div className="container">
                        <div className="breadcrumb-content">
                            <ul className="quiz-nav d-flex flex-wrap align-items-center">
                                <li><Link to={`${urlBase}${refer_url.replace(/&/g, '/')}`}><i className="la la-arrow-left mr-2"></i>Volver</Link></li>                            
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <section className="breadcrumb-area py-5 bg-white pattern-bg">                                
                <div className="container">
                    <div className="breadcrumb-content">
                        <div className="media media-card align-items-center pb-4">
                            <div className="media-img media--img media-img-md rounded-full">
                                <img className="rounded-full lazy" src={datos.imagen_pequena==null ? `${urlBase}/images/avatar_docente.jpg` : `${urlBaseApi}/${datos.imagen_pequena}`} alt="Imagen del usuario" />
                            </div>
                            <div className="media-body">
                                <h2 className="section__title fs-30">{datos.nombres!='' ? datos.nombres+' '+datos.apellidos : <Skeleton width={'30%'} height={25} style={{marginTop: '0px'}}/> }</h2>
                                <span className="d-block lh-18 pt-1 pb-2">{datos.miembro_desde!='' ? `Miembro desde ${datos.miembro_desde}` : <Skeleton width={'25%'} height={20} /> }</span>
                                <p className="lh-18">{datos.profesion!='' ? datos.profesion : <Skeleton width={'30%'} height={20} /> }</p>                                
                            </div>
                        </div>
                        <ul className="social-icons social-icons-styled social--icons-styled">
                            <li>{datos.facebook!=null ? <a href={datos.facebook} target="_blank"><i className="la la-facebook"></i></a> : ''}</li>
                            <li>{datos.twitter!=null ? <a href={datos.twitter} target="_blank"><i className="la la-twitter"></i></a> : ''}</li>
                            <li>{datos.instagram!=null ? <a href={datos.instagram} target="_blank"><i className="la la-instagram"></i></a> : ''}</li>
                            <li>{datos.linkedin!=null ? <a href={datos.linkedin} target="_blank"><i className="la la-linkedin"></i></a> : ''}</li>
                            <li>{datos.youtube!=null ? <a href={datos.youtube} target="_blank"><i className="la la-youtube!"></i></a> : ''}</li>
                        </ul>
                    </div>
                </div>
            </section>
            
            <section className="teacher-details-area pt-50px">
                <div className="container">
                    <div className="student-details-wrap pb-20px">
                        {(datos.docente_descripcion!=null && datos.docente_descripcion!='') ? <h3 className="fs-24 font-weight-semi-bold pb-2">Biografía</h3> : ''}
                        {(datos.docente_descripcion!=null && datos.docente_descripcion!='') ? <p className="pb-5">{datos.docente_descripcion.split('<br />').map((line, index) => (<span key={`bio-parte-${index}`}>{line}<br /></span> ))}</p> : ''}
                        <div className="row">
                            <div className="col-lg-4 responsive-column-half">
                                <div className="counter-item">
                                    <div className="counter__icon icon-element mb-3 shadow-sm">
                                        {datos.es_docente==0 ? <svg className="svg-icon-color-1" width="40" version="1.1" x="0px" y="0px" viewBox="0 0 512 512" xmlSpace="preserve">
                                            <g>
                                                <g>
                                                <g>
                                                ...
                                                <svg className="svg-icon-color-1" width="40" version="1.1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 490.667 490.667" xmlSpace="preserve">
                                                <g>
                                                    <g>
                                                        <path d="M245.333,85.333c-41.173,0-74.667,33.493-74.667,74.667s33.493,74.667,74.667,74.667s74.667-33.493,74.667-74.667
                                                ...
                                                <g>
                                                    <g>
                                                        <path d="M97.515,170.667c-29.419,0-53.333,23.936-53.333,53.333s23.936,53.333,53.333,53.333s53.333-23.936,53.333-53.333
                                                            S126.933,170.667,97.515,170.667z M97.515,256c-17.643,0-32-14.357-32-32c0-17.643,14.357-32,32-32
                                                            c17.643,0,32,14.357,32,32C129.515,241.643,115.157,256,97.515,256z"></path>
                                                    </g>
                                                </g>
                                                <g>
                                                    <g>
                                    ...
                                    <h4 className="counter__title counter text-color-2">{datos.es_docente==0 ? datos.cantidad_comentarios : datos.docente_cantidad_estudiantes}</h4>
                                    <p className="counter__meta">{datos.es_docente==0 ? 'Comentarios' : 'Estudiantes totales'}</p>
                                </div>
                            </div>
                            <div className="col-lg-4 responsive-column-half">
                                <div className="counter-item">
                                    <div className="counter__icon icon-element mb-3 shadow-sm">
                                        <svg className="svg-icon-color-2" width="40" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 512 512" xmlSpace="preserve">
                                            <g>
                                                <g>
                ...
                <div className="bg-gray py-5">
                    <div className="container">
                        <ul className="nav nav-tabs generic-tab justify-content-center" id="myTab" role="tablist">
                            <li className="nav-item" onClick={event =>{ handleCambiarPestana(event, 1) }}>
                                <a className={`nav-link ${pestanaActivada==1 ? 'active': ''}`} id="my-learning-tab" data-toggle="tab" href="#my-learning" role="tab" aria-controls="my-learning" aria-selected="false">
                                    {datos.es_docente==0 ? 'Aprendizaje' : 'Cursos impartiendo'}
                                </a>
                            </li>                            
                            <li className="nav-item" onClick={event =>{ handleCambiarPestana(event, 2) }}>
                                <a className={`nav-link ${pestanaActivada==2 ? 'active': ''}`} id="wishlist-tab" data-toggle="tab" href="#wishlist" role="tab" aria-controls="wishlist" aria-selected="false">
                                    Favoritos
                                </a>
                            </li>
                        </ul>
                        <div className="tab-content pt-40px" id="myTabContent">
                            <div className={`tab-pane fade ${pestanaActivada==1 ? 'show active': ''}`}id="my-learning" role="tabpanel" aria-labelledby="my-learning-tab">
                                <div className="my-course-body">
                                    <div className="my-course-cards">
                                        {(datosCargados && datos.es_docente==0) ? <div className="row">
                                            {Object.keys(cursos).map((key) => (                                
                                                <TarjetaCurso
                                                    key={`tarjeta-e-${cursos[key].id}`}
                                        ...
                                        {(datosCargados && datos.es_docente==1) ? <div className="row">
                                            {Object.keys(cursosInstructor).map((key) => (                                
                                                <TarjetaCurso
                                                    key={`tarjeta-i-${cursosInstructor[key].id}`}
                                                    idcurso={cursosInstructor[key].id}
                                                    url_amigable={cursosInstructor[key].url_amigable}
                                                    nombre={cursosInstructor[key].nombre}
                                                    imagen={cursosInstructor[key].imagen_pequena}
                                                    bestseller={cursosInstructor[key].bestseller}
                                                    promocionado={cursosInstructor[key].promocionado}
                                                    gratis={cursosInstructor[key].gratis}
                                                    alto_valorado={cursosInstructor[key].alto_valorado}
                                                    porcentaje_descuento={cursosInstructor[key].porcentaje_descuento}
                                                    nivel={cursosInstructor[key].nivel}
                                                    instructor={cursosInstructor[key].instructor}
                                                    id_instructor={cursosInstructor[key].id_instructor}
                                                    reviews_puntuacion={cursosInstructor[key].reviews_puntuacion}
                                                    reviews_cantidad={cursosInstructor[key].reviews_cantidad}
                                                    precio_actual={cursosInstructor[key].precio_actual}
                                                    precio_anterior={cursosInstructor[key].precio_anterior}
                                                    favorito={cursosInstructor[key].favorito}
                                                    col_lg={4}
                                                />
                                            ))}      
                                        </div>:''}
                                        {(datosCargados && datos.es_docente==0) ? 
                                        <Paginador elemetosTotales={totalCursos} elementosPorPagina={6} paginaActual={paginaCursos} callbackCambioPagina={setPaginaCursos} />
                                        :''}
                                        {(datosCargados && datos.es_docente==1) ? 
                                        <Paginador elemetosTotales={totalCursosInstructor} elementosPorPagina={6} paginaActual={paginaCursosInstructor} callbackCambioPagina={setPaginaCursosInstructor} />
                                        :''}
                                    </div>
                                </div>
                            </div>
                            <div className={`tab-pane fade ${pestanaActivada==2 ? 'show active': ''}`} id="wishlist" role="tabpanel" aria-labelledby="wishlist-tab">
                                <div className="my-course-body">
                                    <div className="my-course-cards">
                                        <div className="row">
                                            {Object.keys(cursosFavoritos).map((key) => (                                
                                                <TarjetaCurso
                                                    key={`tarjeta-favorito-${cursosFavoritos[key].id}`}
                                                    idcurso={cursosFavoritos[key].id}
                                                    url_amigable={cursosFavoritos[key].url_amigable}
                                                    nombre={cursosFavoritos[key].nombre}
                                                    imagen={cursosFavoritos[key].imagen_pequena}
                                                    bestseller={cursosFavoritos[key].bestseller}
                                                    promocionado={cursosFavoritos[key].promocionado}
                                                    gratis={cursosFavoritos[key].gratis}
                                                    alto_valorado={cursosFavoritos[key].alto_valorado}
                                                    porcentaje_descuento={cursosFavoritos[key].porcentaje_descuento}
                                                    nivel={cursosFavoritos[key].nivel}
                                                    instructor={cursosFavoritos[key].instructor}
                                                    id_instructor={cursosFavoritos[key].id_instructor}
                                                    reviews_puntuacion={cursosFavoritos[key].reviews_puntuacion}
                                                    reviews_cantidad={cursosFavoritos[key].reviews_cantidad}
                                                    precio_actual={cursosFavoritos[key].precio_actual}
                                                    precio_anterior={cursosFavoritos[key].precio_anterior}
                                                    favorito={cursosFavoritos[key].favorito}
                                                    col_lg={4}
                                                />
                                            ))}    
                                        </div>
                                        <Paginador elemetosTotales={totalCursosFavoritos} elementosPorPagina={6} paginaActual={paginaCursosFavoritos} callbackCambioPagina={setPaginaCursosFavoritos} />
                                    </div>
                                </div>
                            </div>
                        </div>
