...
import { mensajesDeError, calcularSegundosDeHorasMinutos } from "./utils";
...
function FormularioCrearExamen() {
  ...
  const [popUpCreado, setPopupCreado] = useState({
    mostrar: false,
    titulo: "",
    contenido: "",
  });
  ...
  useEffect(() => {
    if (idExamenCreado != -1) {
      setPopupCreado({
        ...
        titulo: "Listo",
        contenido:
          "Examen creado, Al presionar aceptar lo enviaremos a la edición de las preguntas.",
      });
    }
  }, [idExamenCreado]);
  ...
  const handleCrearExamen = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${urlBaseApi}/api/examen`, opciones);
      setMostrarSpinner(false);
      const datos = await response.json();
      if (response.ok) {
        setIdExamenCreado(datos.id_examen);
        //el popuop se muestra a través del use effect
        return;
      } else {
        mensajesDeError(
          setPopup,
  ...
  return (
    <>
      ...
      />
      <Popup
        mostrarPopup={popUpCreado.mostrar}
        tamano="xx"
        tipo={2}
        titulo={popUpCreado.titulo}
        mensaje={popUpCreado.contenido}
        funcionAceptar={handleFuncionAceptarPopUpCreado}
        funcionCerrar={handleFuncionCerrarPopUp}
        textoCerrar="Aceptar"
      />
        ...
        <div className="container-fluid">
          <div className="breadcrumb-content d-flex flex-wrap align-items-center justify-content-between mb-3">
            {typeof id_curso !== "undefined" ? (
              <>
                <Link to={`/curso/contenido/${id_curso}`}>
                  ...
                  >
                    <i className="la la-arrow-left mr-1"></i>
                    Atrás
                  </div>
                </Link>
                &nbsp;
            ...
            <div className="media media-card align-items-center theme-title mt-3">
              <h5 className="fs-22 font-weight-semi-bold">Crear examen</h5>
            </div>
          </div>
          <form action="#">
...
export default FormularioCrearExamen;
