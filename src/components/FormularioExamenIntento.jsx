...
function FormularioExamenIntento() {
  ...
  useEffect(() => {
    if (configuracion.cerrado_por_tiempo == 1) {
      ...
      if (
        configuracion.fecha_hora_fin !== null &&
        popUp.data_switch != "tiempo_terminado"
      ) {
  ...
  const obtenerDatosDelServidor = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/examenintento/${id_examen_intento}/${id_curso}`,
        opciones
      );
  ...
  const obtenerDatosCurso = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/curso/informacionBasica/${id_curso}`,
        opciones
      );
      const datos = await response.json();
      if (response.ok) {
  ...
  const handleEnviarRespuestaPregunta = async (
    id_pregunta,
    id_pregunta_opcion_seleccionada
  ) => {
    const raw = {
    ...
    try {
      const response = await fetch(
        `${urlBaseApi}/api/examenRespuestaPregunta/${id_examen_intento}/${id_pregunta}`,
        opciones
      );
      const datos = await response.json();
      if (response.ok) {
  ...
  const handleCerrarIntento = async (cerrado_por_tiempo = false) => {
    ...
    try {
      const response = await fetch(
        `${urlBaseApi}/api/examenintento/cerrarintento/1`,
        opciones
      );
      const datos = await response.json();
      if (response.ok) {
        if (cerrado_por_tiempo) {
          ...
          setPopup({
            mostrar: true,
            titulo: "Respuestas enviadas",
            tipo: 3,
            contenido: "Ahora podrás ver tus resultados.",
            data_switch: "tiempo_terminado",
          });
        }
        //setPopup({mostrar:true, titulo:'Listo', contenido:'Cambios guardados correctamente.'});
      } else {
  ...
  const actualizarOpcionSeleccionada = (
    index,
    id_pregunta_opcion_seleccionada
  ) => {
    ...
    setPreguntas((prevPreguntas) => {
      ...
      if (nuevaPreguntas[index]) {
        if (!actualizado) {
          handleEnviarRespuestaPregunta(
            nuevaPreguntas[index].id_pregunta,
            id_pregunta_opcion_seleccionada
          );
          actualizado = true;
        }
        nuevaPreguntas[index].id_pregunta_opcion_seleccionada =
          id_pregunta_opcion_seleccionada;
      }
  ...
  const handleConfirmarEnviarIntento = (event) => {
    ...
    if (configuracion.tipo_examen != 1) {
      setPopup({
        ...
        titulo: "Confirmar",
        tipo: 3,
        contenido: "Confirma que desea enviar las respuestas?",
        data_switch: "enviar_respuestas",
      });
    } else {
      ...
      if (verificarTodasRespondidas()) {
        ...
        setPopup({
          mostrar: true,
          titulo: "Señala tu respuesta",
          tipo: 2,
          contenido: "Falta alguna respuesta por señalar.",
          data_switch: "",
        });
      }
    }
  ...
  const handleConfirmarEnviarIntentoPorCompletado = () => {
    setPopup({
      mostrar: true,
      titulo: "Deseas enviar las respuestas?",
      tipo: 3,
      contenido:
        "Has terminado de responder, deseas enviar las respuestas ahora?",
      data_switch: "enviar_respuestas",
    });
  };
  ...
  const handleTiempoTerminado = () => {
    setPopup({
      mostrar: true,
      titulo: "Tiempo superado!",
      tipo: 2,
      contenido:
        "El tiempo para responder ha terminado y el intento fue cerrado, las respuestas que hayas seleccionado se procesaron y generaron una calificación.",
      data_switch: "tiempo_terminado",
    });
  };
  ...
  return (
    <>
      ...
      <section className="breadcrumb-area">
          ...
          <div className="container">
            <div className="breadcrumb-content">
              <ul className="quiz-nav d-flex flex-wrap align-items-center">
                <li>
                  <Link to={`${urlBase}/play/${curso.url_amigable}`}>
                    <i className="la la-arrow-left mr-2"></i>Volver al curso
                  </Link>
                </li>
                <li>
                  <div className="d-flex align-items-center">
                    <div className="media media-card">
                      {curso.url_amigable == "" ? (
                        <Skeleton width={82} height={48} />
                      ) : (
                        <Link
                          to={`/play/${curso.url_amigable}`}
                          className="media-img"
                          style={{ height: "auto" }}
                        >
                          {curso.imagen_pequena != null ? (
                            <img
                              src={`${urlBaseApi}/${curso.imagen_pequena}`}
                              alt={curso.nombre}
                            />
                          ) : (
                            <img
                    ...
                    <p>
                      {curso.nombre == "" ? (
                        <Skeleton width={300} style={{ marginLeft: "15px" }} />
                      ) : (
                        <Link to={`/play/${curso.url_amigable}`}>
                          {curso.nombre}
                        </Link>
                      )}
                      {curso.nombre == "" ? (
                        <Skeleton width={150} style={{ marginLeft: "15px" }} />
                      ) : (
                  ...
                  {configuracion.descargables.map((descargable, index) => (
                    <button
                      ...
                      onClick={(event) => {
                        ...
                        handleDescargar(descargable);
                      }}
                    >
                      <i className="la la-paperclip"></i>Descargar archivo:{" "}
                      {descargable.nombre}
                    </button>
                  ))}
                </div>
          ...
          <div className="container">
            ...
            {preguntaActual != -1 && (
              <div className="breadcrumb-content pt-40px">
                <div className="section-heading">
                  {preguntaActual != -1 && Object.keys(preguntas).length > 1 ? (
                    <h2 className="section__title text-white fs-30 pb-2">
                      Pregunta {parseInt(preguntaActual) + 1} de{" "}
        ...
        {preguntaActual != -1 ? (
          <div className="quiz-action-nav bg-light py-3 shadow-sm">
            <div className="container">
              <div className="quiz-action-content d-flex flex-wrap align-items-center justify-content-between">
                <ul className="quiz-nav d-flex align-items-center">
                  {Object.keys(preguntas).length > 1 && (
                    <li>
                      {preguntaActual != -1 ? (
                        <>
                          <i className="la la-sliders fs-17 mr-2"></i>Esta
                          pregunta aquivale al{" "}
                          {preguntas[preguntaActual].porcentaje_puntuacion}%{" "}
                  ...
                  <li>
                    {preguntaActual != -1 ? (
                      <button
                        onClick={(e) => handleConfirmarEnviarIntento(event)}
                        href="#"
                  ...
                  <li>
                    ...
                    configuracion.tiempo_restante != 0 ? (
                      <CountdownClock
                        seconds={configuracion.tiempo_restante}
                        color="#D7D7D7"
                        alpha={0.9}
                        size={50}
                        weight={4}
                ...
                <div className="quiz-nav-btns">
                  {preguntaActual != -1 && preguntaActual > 0 ? (
                    <a
                      onClick={(e) =>
                        cambiarPregunta(parseInt(preguntaActual) - 1)
                      }
                      href="#"
                      className="btn theme-btn theme-btn-transparent mr-2 btn-round"
                    >
          ...
          <div className="quiz-ans-content">
            ...
            <div className="quiz-ans-list py-3">
              {preguntaActual == -1 ? (
                ...
                preguntas[preguntaActual].tipo_pregunta == 2 ? (
                <>
                  {Object.keys(preguntas[preguntaActual].pregunta_opciones).map(
                    (key) => (
                      <div
                        className="custom-control custom-radio mb-1"
                        key={`pregunta-opcion-${preguntas[preguntaActual].pregunta_opciones[key].id}`}
                        ...
                        <input
                          onClick={(e) =>
                            actualizarOpcionSeleccionada(
                              preguntaActual,
                              preguntas[preguntaActual].pregunta_opciones[key]
                                .id
                            )
                          }
                          ...
                          checked={
                            preguntas[preguntaActual]
                              .id_pregunta_opcion_seleccionada ==
                            preguntas[preguntaActual].pregunta_opciones[key].id
                              ? "checked"
                              : ""
                          }
                          type="radio"
                          className="custom-control-input"
                          id={`opcion-${preguntas[preguntaActual].pregunta_opciones[key].id}`}
                        />
                        <label
                          className="custom-control-label custom--control-label"
                          htmlFor={`opcion-${preguntas[preguntaActual].pregunta_opciones[key].id}`}
                        >
                          {preguntas[preguntaActual].pregunta_opciones[
                            key
                          ].texto_opcion
                            .split("<br />")
                            .map((line, index2) => (
            ...
            {preguntaActual == -1 ? (
              ...
              <p className="fs-15">
                <strong className="font-weight-semi-bold text-black">
                  Nota:
                </strong>{" "}
