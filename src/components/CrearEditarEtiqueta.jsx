...
export default function CrearEditarEtiqueta({
  funcionMostrarPopUp,
  id_curso,
  id_categoria,
  id_etiqueta = -1,
  ...
  const handleTag = {
    nombre: (event) => {
      setPopupTag({ ...popUpTag, nombre: event.target.value });
    },
    descripcion: (event) => {
      setPopupTag({ ...popUpTag, descripcion: event.target.value });
    },
    html: (event) => {
      setPopupTag({ ...popUpTag, html: event.target.value });
    },
    show: (event) => {
    ...
    get: async (event) => {
      ...
      try {
        ...
        const response = await fetch(
          `${urlBaseApi}/api/etiqueta/${id_etiqueta}`,
          opciones
        );
          ...
          <div className="modal-content">
            <div className="modal-header border-bottom-gray">
              <div className="pr-2">
                <h5
                  ...
                  id="tagModalTitle"
                >
                  {id_etiqueta != -1 ? "Editar etiqueta" : "Crear etiqueta"}
                </h5>
              </div>
            </div>
            <div className="modal-body">
              <div className="col-lg-12">
                <div className="form-group">
                  ...
                  <input
                    value={popUpTag.nombre}
                    onChange={handleTag.nombre}
              ...
              <div className="col-lg-12">
                <div className="form-group">
                  ...
                  <textarea
                    value={popUpTag.descripcion}
                    onChange={handleTag.descripcion}
              ...
              <div className="col-lg-12">
                <div className="form-group">
                  <label className="label-text">
                    Html (iframes de youtube, vimeo o prezi)
                  </label>
                  <textarea
                    value={popUpTag.html}
                    onChange={handleTag.html}
                    className="form-control form--control user-text-editor pl-3"
                    name="html"
                  ></textarea>
                  {erroresCampos["html"].length > 0 && (
                    <SpamError mensaje={erroresCampos["html"]} />
                  )}
                </div>
              </div>
            </div>
            <div className="modal-footer border-top-gray">
              <button
                ...
                onClick={handleTag.save}
              >
                {id_etiqueta != -1 ? "Guardar" : "Crear"}
              </button>
              <button
                type="button"
                className="btn theme-btn theme-btn-dark mb-2"
                onClick={handleTag.close}
              >
                {" "}
                Cancelar{" "}
              </button>
