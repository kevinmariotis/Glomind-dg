...
function HeaderMenuContent() {
  ...
  useEffect(() => {
    ...
    if (dataTags) {
      ...
      if (
        Math.floor(new Date().getTime() / 1000) -
          parseInt(JSON.parse(dataTags).fechahora) >=
        3600
      ) {
        setCargarTags(true);
      }
  ...
  const obtenerTags = async () => {
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/cursotagagrupacion/getBuscador/1`,
        opciones
      );
                ...
                <div className="menu-wrapper">
                  <div className="menu-category">
                    <ul>
                      <li>
                        <Link to="/">
                          Categorías <i className="la la-angle-down fs-12"></i>
                        </Link>
                        <ul className="cat-dropdown-menu">
                          {Object.keys(datos.datos).map((key) => (
                            <li
                              key={`menusup-categoria-${datos.datos[key].id}`}
                            >
                              <Link
                                to={`${urlBase}/categoria/${datos.datos[key].url_amigable}`}
                              >
                                {datos.datos[key].nombre}{" "}
                                {datos.datos[key].categorias_hijas.length >
                                  0 && <i className="la la-angle-right"></i>}
                              </Link>
                              {}
                              {datos.datos[key].categorias_hijas.length > 0 && (
                                <ul className="sub-menu">
                                  {datos.datos[key].categorias_hijas.map(
                                    (sub_categoria) => {
                                      return (
                      ...
                      <ul>
                        {Object.keys(tags.datos).map((key) => (
                          <li key={`menu-tag-agrupacion-${tags.datos[key].id}`}>
                            <a href="#">
                              {tags.datos[key].nombre}{" "}
                              <i className="la la-angle-down fs-12"></i>
                            </a>
                            {tags.datos[key].tags.length > 0 && (
                              <ul className="dropdown-menu-item">
                                {tags.datos[key].tags.map((sub_tag) => {
                                  return (
                                    <li
                                      key={`menu-subtag-agrupacion-${sub_tag.id}`}
                                    >
                                      <Link
                                        to={`${urlBase}/tag/${sub_tag.url_amigable}`}
                                      >
                                        {sub_tag.nombre}
                                      </Link>
                                    </li>
                  ...
                  <div className="nav-right-button">
                    ...
                    >
                      <i className="la la-user-plus mr-1"></i> Admisión
                    </Link>
                  </div>
                </div>
              </div>
            ...
            <li key={`menu-tag-agrupacion-movil-${tags.datos[key].id}`}>
              ...
              {tags.datos[key].tags.length > 0 && (
                <ul className="sub-menu">
                  {tags.datos[key].tags.map((sub_tag) => {
                    return (
                      <li key={`menu-subtag-agrupacion-movil-${sub_tag.id}`}>
                        <a
                          onClick={() => {
                            handleAbrirTag(sub_tag.url_amigable);
                          }}
                        >
                          {sub_tag.nombre}
                        </a>
              ...
              {datos.datos[key].categorias_hijas.length > 0 && (
                <ul className="sub-menu">
                  {datos.datos[key].categorias_hijas.map((sub_categoria) => {
                    return (
                      <li key={`submenu-categoria-${sub_categoria.id}`}>
                        <a
                          onClick={() => {
                            handleAbrirCategoriaSistema(
                              sub_categoria.url_amigable
                            );
                          }}
                        >
                          {sub_categoria.nombre}
