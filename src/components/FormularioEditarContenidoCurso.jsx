...
function FormularioEditarContenidoCurso() {
    ...
    const [popUpTarea, setPopupTarea] = useState({mostrar:false, id_categoria:-1, id_tarea:-1});
    const [popUpUrl, setPopupUrl] = useState({mostrar:false, id_categoria:-1, id_url:-1});
    ...
    const obtenerNombreContenido = (tipoContenido) => {
        ...
        return contenido ? contenido.nombre : 'Desconocido';
    };
    ...
    return (
        ...
        {popUpTarea.mostrar==1 && <CrearEditarTarea id_curso={id} id_categoria={popUpTarea.id_categoria}  id_tarea={popUpTarea.id_tarea} funcionMostrarPopUp={()=>{ setPopupTarea({...popUpTarea, mostrar:0}); obtenerDatosServidor(); }} />}
        {popUpUrl.mostrar==1 && <CrearEditarUrl id_curso={id} id_categoria={popUpUrl.id_categoria}  id_url={popUpUrl.id_url} funcionMostrarPopUp={()=>{ setPopupUrl({...popUpUrl, mostrar:0}); obtenerDatosServidor(); }} />}
                ...
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <div className="form-group">
                            ...
                            {permissions[92] || esDocente==1 ? <button className="btn theme-btn ml-2" type="button" onClick={()=>{ setPopupTarea({...popUpTarea, mostrar:1, id_tarea:-1, id_categoria:idSeccionAgregarContenido}); setMostrarPopUpAgregarContenido(false); }}> <i className="la la-plus mr-2"></i> Tarea</button> : ''}                                                        
                            {permissions[99] || esDocente==1 ? <button className="btn theme-btn ml-2" type="button" onClick={()=>{ setPopupUrl({...popUpUrl, mostrar:1, id_url:-1, id_categoria:idSeccionAgregarContenido}); setMostrarPopUpAgregarContenido(false); }}> <i className="la la-plus mr-2"></i> Url</button> : ''}
                        </div>
                    </div>
                    <div className="modal-footer border-top-gray">                        
                                            ...
                                            <table className="table generic-table">
                                                ...
                                                <tbody >
                                                    {contenido[key].curso_contenido.map((tema) => 
                                                        <tr key={`contenido-x-${key}-${tema.id_contenido}`}>
                                                            <th scope="row">
                                                                <div className="custom-control custom-checkbox media media-card">                                                                                                                                                                            
                                                                    ...
                                                                    {
                                                                        tema.tipo_contenido == 3 || tema.tipo_contenido == 7 ?
                                                                        <div className="media-img">
                                                                            {
                                                                                tema.ruta_imagen_preview_small && tema.ruta_imagen_preview_small != null ? <img src={`${urlBaseApi}/${tema.ruta_imagen_preview_small}`} /> : <img src={`${urlBase}/images/course-no-image.png`} /> 
                                                                            }
                                                            ...
                                                            <td>
                                                                {obtenerNombreContenido(tema.tipo_contenido)}
                                                            </td>
                                                            <td>
                                                                {tema.nombre}
                                                            </td>
                                                            {permissions[34] || esDocente==1 ? 
                                                                <td>
                                                                    {tema.descargables.map((descargable) => 
                                                                        <div key={`descargable-${descargable.id}`} onClick={()=>{ handleDownload({'ruta_archivo':`${urlBaseApi}/${descargable.ruta_archivo.replace('public/', '')}`, 'nombre_archivo':'descargable.pdf'}) }} className="icon-element icon-element-sm flex-shrink-0 bg-7 mr-3 text-white" style={{cursor:'pointer'}}>
                                                                            <i className="la la-download"></i>
                                                                        </div>                                                                    
                                                                    )}   
                                                                </td>
                                                            ...
                                                            <td>                                                                
                                                                ...
                                                                {(permissions[35] || permissions[36] || esDocente==1) && [1, 2, 5, 6].includes(tema.tipo_contenido) ? <a onClick={(event) => { handleAbrirListaDescargable(event, {id_tipo_contenido:tema.id_tipo_contenido, tipo_contenido:tema.tipo_contenido}) } }  href="#" className="icon-element icon-element-sm shadow-sm cursor-pointer ml-1 text-success" data-toggle="tooltip" data-placement="top" data-title="Editar descargable" title="Editar descargable"><i className="la la-download"></i></a> : ''}
                                                                ...
                                                                {tema.tipo_contenido == 5 && (permissions[93] || esDocente==1) ? <div onClick={()=>{ setPopupTarea({...popUpTarea, mostrar:1, id_tarea:tema.id_tipo_contenido, id_categoria:-1});}} className="icon-element icon-element-sm shadow-sm cursor-pointer m-1 text-secondary" data-toggle="tooltip" data-placement="top" title="Editar Tarea"><span data-toggle="modal" data-target="#resourceEditTarea" className="w-100 h-100 d-inline-block"><i className="la la-cog"></i></span></div>: ''}
                                                                ...
                                                                {tema.tipo_contenido == 7 && (permissions[100] || esDocente==1) ? <div onClick={()=>{ setPopupUrl({...popUpUrl, mostrar:1, id_url:tema.id_tipo_contenido, id_categoria:-1});}} className="icon-element icon-element-sm shadow-sm cursor-pointer m-1 text-secondary" data-toggle="tooltip" data-placement="top" title="Editar Foro"><span data-toggle="modal" data-target="#resourceEditForo" className="w-100 h-100 d-inline-block"><i className="la la-cog"></i></span></div>: ''}
