...
function FormularioDashboardHuecoPreguntas() {
  ...
  const [PopUpAgrupacion, setPopUpAgrupacion] = useState({
    mostrar: false,
    titulo: "",
    contenido: "",
  });
  ...
  const handlePopUpConfirmarBorrarExamenPregunta = (id_examen_pregunta) => {
    setPopupConfirmar({
      mostrar: true,
      titulo: "Confirmar",
      contenido: "Confirma borrar esta pregunta?",
      tipo: "pregunta",
      data: id_examen_pregunta,
    });
  };
  const handlePopUpConfirmarBorrarHuecoPregunta = (id_hueco_pregunta) => {
    setPopupConfirmar({
      mostrar: true,
      titulo: "Confirmar",
      contenido: "Confirma borrar esta pregunta del examen?",
      tipo: "hueco",
      data: id_hueco_pregunta,
    });
  };
  ...
  const handleSetNombreAgrupacion = (event) => {
    setNombreAgrupacion(event.target.value);
  };
  const handlePorcentajeValor = (event) => {
    setPorcentajeValor(event.target.value);
  };
  const handleAgrupacionPreguntaFija = (event) => {
    setAgrupacionPreguntaFija(event.target.value);
  };
  ...
  const obtenerDatosServidor = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/examen/getAgrupacionesPreguntas/${id}`,
        opciones
      );
    ...
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/examen/getPreguntas/${id}`,
        opciones
      );
      setMostrarSpinner(false);
      const datos = await response.json();
  ...
  const obtenerDatosFormularioHueco = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/examenhuecopregunta/getFormularioCrear/${id}${
  ...
  const handleGuardarEditarHueco = async (event) => {
    ...
    if (editandoIdHuecoPregunta == -1) {
      ...
      try {
        setMostrarSpinner(true);
        const response = await fetch(
          `${urlBaseApi}/api/examenhuecopregunta`,
          opciones
        );
        setMostrarSpinner(false);
        const datos = await response.json();
        if (response.ok) {
          ...
          setPopup({
            mostrar: true,
            titulo: "Listo",
            contenido: "Pregunta Fija o Agrupación establecida en el examen.",
          });
          obtenerDatosServidor();
          return;
        } else {
      ...
      try {
        setMostrarSpinner(true);
        const response = await fetch(
          `${urlBaseApi}/api/examenhuecopregunta/${editandoIdHuecoPregunta}`,
          opciones
        );
        setMostrarSpinner(false);
        const datos = await response.json();
  ...
  const handleGuardarEditarAgrupacion = async (event) => {
    ...
    if (editandoIdAgrupacion == -1) {
      ...
      try {
        setMostrarSpinner(true);
        const response = await fetch(
          `${urlBaseApi}/api/examenagrupacionpreguntas`,
          opciones
        );
        setMostrarSpinner(false);
        const datos = await response.json();
      ...
      try {
        setMostrarSpinner(true);
        const response = await fetch(
          `${urlBaseApi}/api/examenagrupacionpreguntas/${editandoIdAgrupacion}`,
          opciones
        );
        setMostrarSpinner(false);
        const datos = await response.json();
  ...
  const handleBorrarAgrupacion = async (id_agrupacion) => {
    ...
    try {
      setMostrarSpinner(true);
      const response = await fetch(
        `${urlBaseApi}/api/examenagrupacionpreguntas/${id_agrupacion}`,
        opciones
      );
      setMostrarSpinner(false);
      const datos = await response.json();
  ...
  const handleBorrarPregunta = async () => {
    ...
    try {
      setMostrarSpinner(true);
      const response = await fetch(
        `${urlBaseApi}/api/examenpregunta/${popUpConfirmar.data}${
          typeof id_curso !== "undefined" ? `/${id_curso}` : ""
        }`,
        opciones
      );
      ...
      if (response.ok) {
        setPopup({
          mostrar: true,
          titulo: "Listo",
          contenido: "Pregunta borrada del banco de preguntas del examen.",
        });
        obtenerDatosServidor();
        return;
      } else {
  ...
  const handleBorrarHuecoPregunta = async () => {
    ...
    try {
      setMostrarSpinner(true);
      const response = await fetch(
        `${urlBaseApi}/api/examenhuecopregunta/${popUpConfirmar.data}${
          typeof id_curso !== "undefined" ? `/${id_curso}` : ""
        }`,
        opciones
      );
      ...
      if (response.ok) {
        setPopup({
          mostrar: true,
          titulo: "Listo",
          contenido: "Pregunta borrada del examen.",
        });
        obtenerDatosServidor();
        return;
      } else {
  ...
  const handleMoverHuecoPregunta = async (
    event,
    id_examen_hueco_pregunta,
    direccion
  ) => {
    ...
    try {
      setMostrarSpinner(true);
      const response = await fetch(
        `${urlBaseApi}/api/examenhuecopregunta/mover/${id_examen_hueco_pregunta}`,
        opciones
      );
      setMostrarSpinner(false);
      const datos = await response.json();
    ...
    <>
      ...
      <div
        className={`modal fade modal-container ${
          popUpCrearHuecoPregunta ? "show" : ""
        }`}
        style={{ background: "rgba(0, 0, 0, 0.7)" }}
        id="crearHuecoPregunta"
        tabIndex="-1"
        role="dialog"
        aria-labelledby="crearHuecoPregunta"
        aria-hidden="true"
      >
        <div
          ...
          <div className="modal-content">
            <div className="modal-header border-bottom-gray">
              <div className="pr-2">
                <h5
                  ...
                  id="crearHuecoPreguntaTitle"
                >
                  {editandoIdHuecoPregunta != -1
                    ? "Editar pregunta establecida"
                    : "Agregar espacio para pregunta"}
                </h5>
              </div>
            </div>
            <div className="modal-body">
              <div className="form-group">
                ...
                <select
                  onChange={handleAgrupacionPreguntaFija}
                  value={agrupacionPreguntaFija}
                  name="id_agrupacion_agregar"
                ...
                >
                  ...
                  {preguntasDisponibles.map((tema) => (
                    <option key={tema.id} value={tema.id}>
                      P.Fija:{" "}
                      {tema.texto_pregunta.length > 100
                        ? tema.texto_pregunta
                            .slice(0, 100)
                            .replace(/<br \/>/g, " ") + "..."
                        : tema.texto_pregunta.replace(/<br \/>/g, " ")}
                    </option>
                  ))}
                </select>
                {erroresCampos["id_examen_agrupacion"].length > 0 && (
                  <SpamError mensaje={erroresCampos["id_examen_agrupacion"]} />
                )}
              </div>
              <div className="form-group">
                <label className="label-text">Porcentaje en el examen</label>
                <select
                  onChange={handlePorcentajeValor}
                ...
                {erroresCampos["porcentaje_valor"].length > 0 && (
                  <SpamError mensaje={erroresCampos["porcentaje_valor"]} />
                )}
              </div>
            </div>
            <div className="modal-footer border-top-gray">
              <button
                ...
                onClick={handleGuardarEditarHueco}
              >
                {" "}
                Guardar{" "}
              </button>
      ...
      <div
        className={`modal fade modal-container ${
          PopUpAgrupacion.mostrar == true ? "show" : ""
        }`}
        style={{ background: "rgba(0, 0, 0, 0.7)" }}
        id="crearAgrupacion"
        tabIndex="-1"
        role="dialog"
        aria-labelledby="crearAgrupacionTitle"
        aria-hidden="true"
      >
        <div
          className="modal-dialog modal-lg modal-dialog-centered modal-theme"
          ...
          <div className="modal-content">
            <div className="modal-header border-bottom-gray">
              <div className="pr-2">
                <h5
                  className="modal-title fs-19 font-weight-semi-bold lh-24"
                  id="crearAgrupacionTitle"
                >
                  {editandoIdAgrupacion != -1
                    ? "Editar agrupación"
                    : "Crear agrupación"}
                </h5>
              </div>
            ...
            <div className="modal-footer border-top-gray">
              <button
                type="button"
                className="btn theme-btn mb-2"
                onClick={handleGuardarEditarAgrupacion}
              >
                {" "}
                Guardar{" "}
              </button>
      ...
      <div
        className={`modal fade modal-container ${
          popUpCrearPregunta ? "show" : ""
        }`}
        style={{ background: "rgba(0, 0, 0, 0.7)" }}
        id="crearPregunta"
        tabIndex="-1"
        role="dialog"
        aria-labelledby="crearPreguntaTitle"
        aria-hidden="true"
      >
        <div
          className="modal-dialog modal-lg modal-dialog-centered modal-theme"
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <br />
                <Link
                  to={`${urlBase}/examen/crearpregunta/falso_verdadero/${id}${
                    typeof id_curso !== "undefined" ? `/${id_curso}` : ""
                  }`}
                  className="btn theme-btn btn-round"
                  type="button"
                >
                  <i className="la la-plus mr-2"></i>Falso o verdadero
                </Link>
                &nbsp;
                <Link
                  to={`${urlBase}/examen/crearpregunta/seleccion_multiple_unica_respuesta/${id}${
                    typeof id_curso !== "undefined" ? `/${id_curso}` : ""
                  }`}
                  className="btn theme-btn btn-round"
                  type="button"
                >
                  <i className="la la-plus mr-2"></i>Selección múltiple, única
                  respuesta
                </Link>
              </div>
            </div>
            ...
            <div className="card-theme">
              <div className="card-body">
                  ...
                  <div className="col-lg-12">
                    {mostrarMensaje100 ? (
                      <div className="alert alert-warning" role="alert">
                        La suma de los porcentajes de todas las preguntas deben
                        dar 100%
                      </div>
                    ) : (
                      ""
                        ...
                        <tbody>
                          {huecos.map((tema) => (
                            <tr key={`contenido-x-${tema.id}`}>
                              <th scope="row">{tema.posicion + 1}</th>
                              <th scope="row">
                                {tema.pregunta_fija == 1 ? (
                              ...
                              <td>
                                <div style={{ display: "flex" }}>
                                  ...
                                  <div
                                    onClick={() => {
                                      handleEditarHuecoPregunta({
                                        id_hueco_pregunta: tema.id,
                                        id_examen_agrupacion:
                                          tema.id_agrupacion,
                                        porcentaje_valor: tema.porcentaje_valor,
                                      });
                                    }}
                                  ...
                                  <div
                                    onClick={() => {
                                      handlePopUpConfirmarBorrarHuecoPregunta(
                                        tema.id
                                      );
                                    }}
                ...
                <button
                  ...
                  type="submit"
                  onClick={handleAgregarHuecoPregunta}
                >
                  <i className="la la-plus mr-2"></i> Agregar espacio para
                  pregunta
                </button>
                &nbsp;
              </div>
            ...
            <div className="card-theme mt-4">
              <div className="card-body">
                            ...
                            <tr key={`contenido-agrupacion-${tema.id}`}>
                              ...
                              <td>
                                <div style={{ display: "flex" }}>
                                  ...
                                  <div
                                    onClick={() => {
                                      handleBorrarAgrupacion(tema.id);
                                    }}
                                    // className="icon-element icon-element-sm shadow-sm cursor-pointer ml-1 text-danger"
                                    data-toggle="tooltip"
                                    data-placement="top"
                ...
                <button
                  ...
                  style={{ marginTop: "20px" }}
                  type="submit"
                  onClick={handleAgregarAgrupacion}
                >
                  <i className="la la-plus mr-2"></i> Crear agrupación
                </button>
              </div>
            </div>
            <div className="card-theme mt-4">
              <div className="card-body">
                ...
                <div className="row">
                  <div className="col-lg-12">
                    <div className="table-responsive">
                      <table className="table generic-table text-center">
                        <thead>
                          <tr>
                        ...
                        <tbody>
                          {preguntas.map((tema) => (
                            <tr key={`contenido-x-${tema.id}`}>
                              ...
                              <td>
                                <div style={{ display: "flex" }}>
                                  {tema.tipo_pregunta == 1 ? (
                                    <Link
                                      to={`${urlBase}/examen/editarpregunta/seleccion_multiple_unica_respuesta/${id}/${
                                        tema.id
                                      }${
                                        typeof id_curso !== "undefined"
                                          ? `/${id_curso}`
                                  ...
                                  {tema.tipo_pregunta == 2 ? (
                                    <Link
                                      to={`${urlBase}/examen/editarpregunta/falso_verdadero/${id}/${
                                        tema.id
                                      }${
                                        typeof id_curso !== "undefined"
                                          ? `/${id_curso}`
                                  ...
                                  <div
                                    onClick={() => {
                                      handlePopUpConfirmarBorrarExamenPregunta(
                                        tema.id
                                      );
                                    }}
                                    // className="icon-element icon-element-sm shadow-sm cursor-pointer ml-1 text-danger"
                ...
                >
                  <i className="la la-plus mr-2"></i> Crear pregunta
                </button>
              </div>
            </div>
            <div className="course-submit-btn-box pb-4"></div>
          </form>
          <DashboardFooter />
        </div>
      </div>
