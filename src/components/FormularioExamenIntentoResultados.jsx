...
function FormularioExamenIntentoResultados() {
  ...
  const [configuracion, setConfiguracion] = useState({
    fecha_hora_fin_formateada: "",
    duracion_realizacion: "",
    calificacion: "",
    mostrar_retroalimentacion: 0,
    politica_retroalimentacion: 0,
  });
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (datos.configuracion.fecha_hora_fin === null) {
          navigate(
            `/examen/presentacion/${datos.configuracion.id_examen}/${id_curso}`
          );
        }
      } else {
        mensajesDeError(
  ...
  const handleConfirmarIntento = (event) => {
    ...
    setPopup({
      ...
      titulo: "Confirmar",
      tipo: 3,
      contenido: "Confirma que desea iniciar un intento?",
      data_switch: "iniciar_intento",
    });
  };
  ...
  const handleBorrarIntento = (event) => {
    ...
    setPopup({
      ...
      titulo: "Confirmar",
      tipo: 3,
      contenido: "Confirma que desea borrar este intento?",
      data_switch: "borrar_intento",
    });
  };
    ...
    <>
              ...
              <div className="breadcrumb-content text-center">
                <div className="section-heading">
                  ...
                  <h2 className="section__title text-white pt-2">
                    Tu calificación es: {configuracion.calificacion}
                  </h2>
                </div>
        ...
        <div className="quiz-action-nav bg-white py-3 shadow-sm">
          <div className="container">
            <div className="quiz-action-content d-flex flex-wrap align-items-center justify-content-between">
              <ul className="quiz-nav d-flex flex-wrap align-items-center">
                <li>
                  {configuracion.calificacion >= 3 ? (
                    <i className="la la-check-circle fs-17 mr-2"></i>
                  ) : (
                    <i className="la la-close fs-17 mr-2"></i>
                  )}
                  {configuracion.calificacion}/5.0 Puntuación
                </li>
                <li>
                  <i className="la la-clock fs-17 mr-2"></i>
                  {configuracion.duracion_realizacion}
                </li>
                <li>
                  <i className="la la-bar-chart fs-17 mr-2"></i>
                  {nivelHabilidad[configuracion.tipo_examen]}
                </li>
                {curso.es_docente == 1 && (
                  <li>
                    <button
                      onClick={handleBorrarIntento}
                      className="btn theme-btn theme-btn-transparent text-white-50 mb-2"
                    >
                      {" "}
                      Borrar intento
      ...
      {configuracion.mostrar_retroalimentacion == 1 ? (
        Object.keys(preguntas).map((key) => (
          <section
            key={`pregunta-retro-${key}`}
            className="quiz-ans-wrap pt-60px pb-60px"
          >
            <div className="container">
              <div className="quiz-ans-content">
                <div className="d-flex align-items-center">
                  <span className="icon-element icon-element-sm mr-2 bg-1 text-white">
                    {parseInt(key) + 1}
                  </span>
                  <h3 className="fs-22 font-weight-semi-bold">
                    Pregunta {parseInt(key) + 1} de{" "}
                    {Object.keys(preguntas).length}
                  </h3>
                  &nbsp;
                  <p className="section__desc text-white-50">
                    |{" "}
                    {preguntas[key].porcentaje_puntuacion != 100
                      ? `${preguntas[key].porcentaje_puntuacion}% - ${preguntas[key].calificacion}`
                      : ""}
                  </p>
                </div>
                <p className="pt-2">
                  {preguntas[key].texto_pregunta
                    .split("<br />")
                    .map((line, index) => (
                      <span key={index}>
                        {line}
                ...
                <ul className="quiz-result-list pt-4 pl-3">
                  {Object.keys(preguntas[key].pregunta_opciones).map((key2) => (
                    <li
                      key={`pregunta-op${key}-${key2}`}
                      className="text-black mb-2"
                    >
                      {preguntas[key].id_pregunta_opcion_seleccionada ==
                        preguntas[key].pregunta_opciones[key2].id &&
                      preguntas[key].pregunta_opciones[key2]
                        .porcentaje_puntuacion > 0 ? (
                        ...
                        <span className="icon-element icon-element-xs mr-2 border border-gray">
                          {abecedario[key2 <= 25 ? key2 : 25]}
                        </span>
                      )}
                      {preguntas[key].pregunta_opciones[key2].texto_opcion
                        .split("<br />")
                        .map((line, index2) => (
                          <span key={index2}>
                            {line}
                ...
                {parseInt(preguntas[key].calificacion) != 0 ? (
                  preguntas[key].texto_retro_afirmativa != null ? (
                    <p className="section__desc text-white-50">
                      {preguntas[key].texto_retro_afirmativa
                        .split("<br />")
                        .map((line, index) => (
                          <span key={index}>
                            {line}
                ...
                ) : preguntas[key].texto_retro_negativa != null ? (
                  <p className="section__desc text-white-50">
                    {preguntas[key].texto_retro_negativa
                      .split("<br />")
                      .map((line, index) => (
                        <span key={index}>
                          {line}
        ...
        <div className="bg-dark pt-60px pb-60px">
          <div className="container">
            <div className="breadcrumb-content text-center">
              <div className="section-heading">
                <p className="section__desc text-white-50">
                  Las retroalimentaciones no están disponibles
                  {configuracion.politica_retroalimentacion == 3
                    ? " hasta que completes todos los intentos."
                    : "  en este momento."}
                </p>
              </div>
            </div>
      ...
      <div className="quiz-action-nav bg-white py-3 shadow-sm">
        <div className="container">
          <div className="quiz-action-content d-flex flex-wrap align-items-center justify-content-between">
            <div className="breadcrumb-btn-box pt-30px">
              <Link
                to={`${urlBase}/play/${curso.url_amigable}`}
                className="btn theme-btn theme-btn-transparent text-white-50 mr-2 mb-2"
              >
                Volver al curso
              </Link>
