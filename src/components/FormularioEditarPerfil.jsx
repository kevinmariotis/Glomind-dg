...
function FormularioEditarPerfil() {
  ...
  const [datosUsuario, setDatosUsuario] = useState({
    docente_rating: 99.9,
    docente_reviews: 0,
  }); //datos estaticos que no se editarán
  ...
  const [advertenciaEmailMostrada, setAdvertenciaEmailMostrada] =
    useState(false);
  ...
  const [formConfirmarNuevaContrasena, setFormConfirmarNuevaContrasena] =
    useState("");
  const [formContrasenaEliminarCuenta, setFormContrasenaEliminarCuenta] =
    useState("");
  ...
  useEffect(() => {
    if (
      formEmail != "" &&
      formEmailOriginal != "" &&
      formEmail != formEmailOriginal &&
      !advertenciaEmailMostrada
    ) {
      ...
      setPopup({
        ...
        titulo: "Advertencia",
        contenido:
          "Al guardar otro email se cerrará la sesión y deberá valiarlo de nuevo, asegúrese que está escribiendo el nuevo email correctamente.",
      });
    }
  }, [formEmail]);
  ...
  const handleNombresChange = (event) => {
    setFormNombres(event.target.value);
  };
  const handleApellidosChange = (event) => {
    setFormApellidos(event.target.value);
  };
  ...
  const handleDepartamentoChange = (event) => {
    setFormIdDepartamento(event.target.value);
  };
  const handleCiudadChange = (event) => {
    setFormCiudad(event.target.value);
  };
  const handleCedulaChange = (event) => {
    setFormCedula(event.target.value);
  };
  ...
  const handleProfesionChange = (event) => {
    setFormProfesion(event.target.value);
  };
  const handleFacebookChange = (event) => {
    setFormFacebook(event.target.value);
  };
  const handleTwitterChange = (event) => {
    setFormTwitter(event.target.value);
  };
  const handleInstagramChange = (event) => {
    setFormInstagram(event.target.value);
  };
  const handleLinkedinChange = (event) => {
    setFormLinkedin(event.target.value);
  };
  const handleYoutubeChange = (event) => {
    setFormYoutube(event.target.value);
  };

  const handleContrasenActualChange = (event) => {
    setFormContrasenaActual(event.target.value);
  };
  const handleNuevaContrasenaChange = (event) => {
    setFormNuevaContrasena(event.target.value);
  };
  const handleConfirmarNuevaContrasenaChange = (event) => {
    setFormConfirmarNuevaContrasena(event.target.value);
  };

  const handleContrasenaEliminarCuenta = (event) => {
    setFormContrasenaEliminarCuenta(event.target.value);
  };
  ...
  const obtenerDatosDelServidor = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/usuario/${id != undefined ? id : ""}`,
        opciones
      );
      ...
      if (response.ok) {
        ...
        if (datos.usuario.docente_descripcion != null) {
          biografia_array =
            datos.usuario.docente_descripcion.split("<separador>");
        }
        let biografiax = "";
  ...
  const handleActualizarPerfil = async (event) => {
    ...
    if (formNombres != null) {
      raw.nombres = formNombres;
    }
    if (formApellidos != null) {
      raw.apellidos = formApellidos;
    }
    if (formEmail != null) {
      raw.email = formEmail;
    }
    if (formTelefono != null) {
      raw.telefono = formTelefono.toString();
    }
    if (formIdPais != null) {
      raw.id_pais = formIdPais;
    }
    if (formIdDepartamento != null) {
      raw.id_departamento = formIdDepartamento;
    }
    if (formCiudad != null) {
      raw.ciudad = formCiudad;
    }
    if (formProfesion != null) {
      raw.profesion = formProfesion;
    }
    if (formRecibirPromociones != null) {
      raw.recibir_promociones = formRecibirPromociones;
    }
    ...
    try {
      setMostrarSpinner(true);
      const response = await fetch(
        `${urlBaseApi}/api/usuario/${id != undefined ? id : "0"}`,
        opciones
      );
      setMostrarSpinner(false);
      const datos = await response.json();
      if (response.ok) {
        ...
        if (imagenSeleccionada != null) {
          ...
          const response = await fetch(
            `${urlBaseApi}/api/usuario/actualizarImagen/${
              id != undefined ? id : "0"
            }`,
            opciones
          );
          ...
          if (response.ok) {
            setPopup({
              mostrar: true,
              titulo: "Listo",
              contenido: "Datos guardados satisfactoriamente.",
              data_switch: cerrar_sesion ? "cerrar-sesion" : "",
            });
            obtenerDatosDelServidor();
            return;
          } else {
          ...
          setPopup({
            ...
            titulo: "Listo",
            contenido: "Datos guardados satisfactoriamente.",
            data_switch: datos.cerrar_sesion == "1" ? "cerrar-sesion" : "",
          });
        }
        return;
      } else {
  ...
  const handleActualizarRedesSociales = async (event) => {
    ...
    try {
      setMostrarSpinner(true);
      const response = await fetch(
        `${urlBaseApi}/api/usuario/editarRedesSociales/${
          id != undefined ? id : "0"
        }`,
        opciones
      );
      ...
      if (response.ok) {
        setPopup({
          mostrar: true,
          titulo: "Listo",
          contenido: "Redes sociales guardadas satisfactoriamente.",
        });
        return;
      } else {
        mensajesDeError(
  ...
  const handleActualizarContrasena = async (event) => {
    ...
    try {
      setMostrarSpinner(true);
      const response = await fetch(
        `${urlBaseApi}/api/usuario/cambiarcontrasena/${
          id != undefined ? id : "0"
        }`,
        opciones
      );
      ...
      if (response.ok) {
        ...
        setPopup({
          mostrar: true,
          titulo: "Listo",
          contenido: "Contraseña cambiada exitosamente.",
        });
        return;
      } else {
        mensajesDeError(
  ...
  const eliminarCuenta = async (event) => {
    ...
    try {
      if (id == undefined) {
        //bloqueo para solo eliminar su propia cuenta.
        setMostrarSpinner(true);
        const response = await fetch(
          `${urlBaseApi}/api/usuario/eliminarCuenta/0`,
          opciones
        );
        setMostrarSpinner(false);
        const datos = await response.json();
        if (response.ok) {
          setPopup({
            ...
            titulo: "Listo",
            contenido:
              "Cuenta eliminada correctamente, gracias por usar nuestros servicios.",
          });
          return;
        } else {
          mensajesDeError(
  ...
  const getDepartamentos = (id_pais) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const response = await fetch(
          `${urlBaseApi}/api/pais/getDepartamentos/${id_pais}`,
          opciones
        );
        const data = await response.json();
    ...
    <>
      ...
      <div
        className={`modal fade modal-container ${
          verPopUpEliminarCuenta == true ? "show" : ""
        }`}
        style={{ background: "rgba(0, 0, 0, 0.7)" }}
        id="eliminarCuentta"
        tabIndex="-1"
        role="dialog"
        aria-labelledby="eliminarCuentaTitle"
        aria-hidden="true"
      >
        <div
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <div className="form-group">
                <label className="label-text">
                  Confirme esta acción escribiendo su contraseña
                </label>
                <input
                  onChange={handleContrasenaEliminarCuenta}
                  value={formContrasenaEliminarCuenta}
                  className="form-control form--control pl-3"
                  type="password"
                  name="contasena_eliminar_cuenta"
                  maxLength="32"
                  placeholder=""
                />
                {erroresCampos["contrasena"].length > 0 && (
                  <SpamError mensaje={erroresCampos["contrasena"]} />
                )}
              </div>
            </div>
            <div className="modal-footer border-top-gray">
              ...
              <button
                ...
                onClick={() => {
                  setVerPopUpEliminarCuenta(false);
                }}
              >
                {" "}
                Cancelar{" "}
        ...
        <div className="container-fluid edit-perfil-content">
                ...
                <div className="media media-card align-items-center">
                  <div className="media-img media--img media-img-md rounded-full">
                    <img
                      ...
                      src={
                        datosUsuario.imagen_pequena == null
                          ? `${urlBase}/images/avatar_docente.jpg`
                          : `${urlBaseApi}/${datosUsuario.imagen_pequena}`
                      }
                  ...
                  <div className="media-body">
                    ...
                    <div className="rating-wrap d-flex align-items-center pt-2">
                      {datosUsuario.docente_reviews > 0 && (
                        <div className="rating-wrap d-flex align-items-center pt-2">
                          <div className="review-stars">
                            <span className="rating-number">
                              {datosUsuario.docente_rating}
                            </span>
          ...
          <div className="tab-content" id="myTabContent">
                  ...
                  <div className="media-body">
                    <div {...getRootProps()}>
                      ...
                      <span className="file-upload-text">
                        <i className="la la-photo mr-2"></i>Subir foto
                      </span>
                    </div>
                    <ul>{fileList}</ul>
                    <p className="fs-14">
                      El tamaño máximo de archivo es 2 MB, dimensión mínima:
                      200x200 y los archivos adecuados son .jpg y .png.
                    </p>
                  </div>
                </div>
                <form method="post" className="row pt-40px">
                  <div className="input-box col-lg-12">
                    ...
                    <div className="form-group">
                      <input
                        ...
                        value={formProfesion}
                      />
                      {erroresCampos["profesion"].length > 0 && (
                        <SpamError mensaje={erroresCampos["profesion"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["nombres"].length > 0 && (
                        <SpamError mensaje={erroresCampos["nombres"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      />
                      <span className="la la-user input-icon"></span>
                      {erroresCampos["apellidos"].length > 0 && (
                        <SpamError mensaje={erroresCampos["apellidos"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["email"].length > 0 && (
                        <SpamError mensaje={erroresCampos["email"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["telefono"].length > 0 && (
                        <SpamError mensaje={erroresCampos["telefono"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["id_pais"].length > 0 && (
                        <SpamError mensaje={erroresCampos["id_pais"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["id_departamento"].length > 0 && (
                        <SpamError mensaje={erroresCampos["id_departamento"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      <input
                        ...
                        value={formCiudad}
                      />
                      {erroresCampos["ciudad"].length > 0 && (
                        <SpamError mensaje={erroresCampos["ciudad"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      <input
                        ...
                        value={formCedula}
                      />
                      {erroresCampos["identificacion"].length > 0 && (
                        <SpamError mensaje={erroresCampos["identificacion"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-12">
                    ...
                    <div className="form-group">
                      <textarea
                        ...
                        value={formBiografia !== null ? formBiografia : ""}
                        rows="8"
                        className="form-control form--control user-text-editor pl-3"
                        name="biografia"
                      ></textarea>
                      {erroresCampos["docente_descripcion"].length > 0 && (
                        <SpamError
                          mensaje={erroresCampos["docente_descripcion"]}
                        />
                      )}
                    </div>
                  </div>
            ...
            <div
              ...
              id="edit-reders"
              role="tabpanel"
              aria-labelledby="edit-redes-tab"
            >
              <div className="setting-body">
                <h3 className="fs-17 font-weight-semi-bold pb-4">
                ...
                <form method="post" className="row">
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["facebook"].length > 0 && (
                        <SpamError mensaje={erroresCampos["facebook"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["twitter"].length > 0 && (
                        <SpamError mensaje={erroresCampos["twitter"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["instagram"].length > 0 && (
                        <SpamError mensaje={erroresCampos["instagram"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["linkedin"].length > 0 && (
                        <SpamError mensaje={erroresCampos["linkedin"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-6">
                    ...
                    <div className="form-group">
                      ...
                      {erroresCampos["youtube"].length > 0 && (
                        <SpamError mensaje={erroresCampos["youtube"]} />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-12 py-2">
                    <button
                      onClick={handleActualizarRedesSociales}
                      type="submit"
                      className="btn theme-btn btn-round"
                    >
                      Guardar cambios
              ...
              <div className="setting-body">
                ...
                <form method="post" className="row">
                  <div className="input-box col-lg-4">
                    ...
                    <div className="form-group">
                      <input
                        onChange={handleContrasenActualChange}
                        maxLength={32}
                        className="form-control form--control"
                        type="password"
                        name="text"
                      ...
                      {erroresCampos["contrasena_actual"].length > 0 && (
                        <SpamError
                          mensaje={erroresCampos["contrasena_actual"]}
                        />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-4">
                    ...
                    <div className="form-group">
                      ...
                      />
                      <span className="la la-lock input-icon"></span>
                      {erroresCampos["nueva_contrasena"].length > 0 && (
                        <SpamError
                          mensaje={erroresCampos["nueva_contrasena"]}
                        />
                      )}
                    </div>
                  </div>
                  <div className="input-box col-lg-4">
                    ...
                    <div className="form-group">
                      ...
                      />
                      <span className="la la-lock input-icon"></span>
                      {erroresCampos["confirmar_nueva_contrasena"].length >
                        0 && (
                        <SpamError
                          mensaje={erroresCampos["confirmar_nueva_contrasena"]}
                        />
                      )}
                    </div>
                  </div>
            ...
            >
              <div className="setting-body">
                ...
                <div className="danger-zone pt-40px">
                  <h4 className="fs-17 font-weight-semi-bold text-danger">
                    Borrar cuenta permanentemente
                  </h4>
                  <p className="pt-1 pb-4">
                    <span className="text-warning">Advertencia: </span>Una vez
                    que eliminas tu cuenta, no hay vuelta atrás. Por favor
                    asegúrese.
                  </p>
                  <button
