...
function FormularioEditarExamenPreguntaFv() {
  ...
  const handleFalsoVerdaderoChange = (event) => {
    setFalsoVerdadero(event.target.value);
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        const response2 = await fetch(
          `${urlBaseApi}/api/examenpregunta/${id_examen_pregunta}`,
          opciones
        );
        setMostrarSpinner(false);
        ...
        if (response.ok) {
          if (datos2.pregunta.id_examen == id) {
            ...
            const desc2_array =
              datos2.pregunta.texto_retro_afirmativa.split("<br />");
            let desc2 = "";
            ...
            const desc3_array =
              datos2.pregunta.texto_retro_negativa.split("<br />");
            let desc3 = "";
            ...
            setPopup({
              mostrar: true,
              titulo: "Error",
              contenido: "El examen no corresponde a la pregunta.",
            });
          }
        } else {
          mensajesDeError(
  ...
  const handleBorrarMedia = async (event) => {
    ...
    setMostrarSpinner(true);
    const response = await fetch(
      `${urlBaseApi}/api/examenpregunta/borrarMedia/${id_examen_pregunta}`,
      opcionesArchivo
    );
    setMostrarSpinner(false);
    if (response.ok) {
  ...
  const handleEditarPregunta = async (event) => {
    ...
    try {
      setMostrarSpinner(true);
      const response = await fetch(
        `${urlBaseApi}/api/examenpregunta/editarpreguntacompleta/${id_examen_pregunta}${
      ...
      if (response.ok) {
        ...
        if (file) {
          ...
          const response = await fetch(
            `${urlBaseApi}/api/examenpregunta/actualizarMedia/${id_examen_pregunta}`,
            opcionesArchivo
          );
          if (!response.ok) {
            setMostrarSpinner(false);
        ...
        setPopup({
          mostrar: true,
          titulo: "Listo",
          contenido: "Pregunta guardada correctamente.",
        });
        return;
      } else {
        mensajesDeError(
  ...
  const renderMedia = () => {
    ...
    if (["png", "jpeg", "jpg"].includes(fileExtension)) {
      return (
        <img src={processedMedia} alt="Imagen" style={{ maxWidth: "300px" }} />
      );
    }
    ...
    <>
      ...
      <div className="dashboard-content-wrap">
        <div className="container-fluid">
          <div className="breadcrumb-content d-flex flex-wrap align-items-center justify-content-between mb-3">
            ...
            <div className="media media-card align-items-center theme-title mt-3">
              <h3 className="fs-22 font-weight-semi-bold">
                Editar pregunta de falso o verdadero
              </h3>
              <h5>&nbsp;|&nbsp;{nombreexamen}</h5>
            </div>
          ...
          <form action="#">
            {erroresCampos["id_examen_pregunta"].length > 0 && (
              <SpamError mensaje={erroresCampos["id_examen_pregunta"]} />
            )}
                ...
                <div className="row">
                  ...
                  <div className="col-lg-12">
                    <div className="form-group">
                      ...
                      <select
                        onChange={handleFalsoVerdaderoChange}
                        value={falsoVerdadero}
                        name="respuesta"
                        className={`form-control ${
                          temaActual == 1 ? "" : "select-dark"
                        }`}
                      ...
                      {erroresCampos["respuesta"].length > 0 && (
                        <SpamError mensaje={erroresCampos["respuesta"]} />
                      )}
                    </div>
                  </div>
                  <div className="col-lg-12">
                    <div className="form-group">
                      ...
                      {media != null && (
                        <button
                          ...
                          type="button"
                          onClick={handleBorrarMedia}
                        >
                          <i className="la la-trash mr-2"></i> Borrar media
                        </button>
                      )}
                      <input
                        type="file"
