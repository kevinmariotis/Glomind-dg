  ...
  const obtenerDatosServidor = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${urlBaseApi}/api/curso/getCalendario/${id_curso}`,
        opciones
      );
      if (response.ok) {
        ...
        let eventos = [];
        datos.forEach((rango) => {
          let fecha_inicio_formateada = rango.fecha_hora_inicio.replace(
            " ",
            "T"
          );
          ...
          var evento = {
            ...
            tipo_contenido: rango.tipo_contenido,
    ...
    return (
      <>
        <div
          style={{
            backgroundColor:
              coloresEventos[eventInfo.event.extendedProps.tipo_contenido],
            borderRadius: "10px",
            margin: "0 5px",
            padding: "0 10px",
            fontSize: "12px",
        ...
        >
          {" "}
          {eventInfo.event.extendedProps.tipo_evento}: {eventInfo.event.title}
        </div>
        {/* <br />   */}
      </>
    );
          ...
          <FullCalendar
            ...
            dayCellContent={(arg) => {
              ...
              return (
                <div
                  style={{
                    backgroundColor:
                      today === cellDate ? "#d0e0ff" : "transparent", // Color de fondo si es el día actual
                    padding: "5px",
                    borderRadius: "5px",
                    color:
                      today === cellDate
            ...
            eventClick={(info) => {
              setPopup({
                ...
                mostrar: true,
                tipo: 3,
                titulo: "Abrir la actividad?",
                data_switch: "abrir_curso_contenido",
                contenido: `${info.event.title}, ${info.event.extendedProps.fecha_hora_inicio_esp} a ${info.event.extendedProps.fecha_hora_fin_esp}`,
                data_id: info.event.extendedProps.id_curso_contenido,
                data_id2: info.event.extendedProps.url_navegar,
              });
            }}
          />
        </div>
