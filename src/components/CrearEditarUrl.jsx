...
export default function CrearEditarUrl({
  funcionMostrarPopUp,
  id_curso,
  id_categoria,
  id_url = -1,
  ...
  const [popUpObjeto, setPopupObjeto] = useState({
    ...
    descripcion: "",
    url: "",
    archivo_vista_previa: null,
    ruta_imagen_preview_small: null,
    id_curso: -1,
    id_categoria: -1,
  });
  ...
  const handleObjeto = {
    nombre: (event) => {
      setPopupObjeto({ ...popUpObjeto, nombre: event.target.value });
    },
    descripcion: (event) => {
      setPopupObjeto({ ...popUpObjeto, descripcion: event.target.value });
    },
    url: (event) => {
      setPopupObjeto({ ...popUpObjeto, url: event.target.value });
    },
    show: (event) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setPopupObjeto({
            ...
            url: datos.url,
            archivo_vista_previa: null,
            ruta_imagen_preview_small: datos.ruta_imagen_preview_small,
          });
        } else {
      ...
      if (id_url != -1) {
        ...
        if (preview) {
          ...
          const responsePreview = await fetch(
            `${urlBaseApi}/api/url/actualizarImagenVistaPrevia/${id_url}`,
            opcionesPreview
          );
          setMostrarSpinner(false);
            ...
            <div className="modal-body">
              ...
              <div className="col-lg-12">
                <div className="form-group">
                  ...
                  <input
                    type="text"
                    value={popUpObjeto.url}
                    onChange={handleObjeto.url}
                    maxLength="2048"
                    className="form-control form--control user-text-editor pl-3"
                    name="url"
                  />
                  {erroresCampos["url"].length > 0 && (
                    <SpamError mensaje={erroresCampos["url"]} />
                  )}
                </div>
              </div>
              <div className="col-lg-12">
