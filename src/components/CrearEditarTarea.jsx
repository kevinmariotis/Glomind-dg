...
export default function CrearEditarTarea({
  funcionMostrarPopUp,
  id_curso,
  id_categoria,
  id_tarea = -1,
  ...
  const [datos, setDatos] = useState({
    ...
    hora_fin: "",
    minuto_fin: "",
    reenviar_post_calificacion: -1,
    porcentaje_en_total_curso: -1,
    id_curso: -1,
    id_categoria: -1,
  ...
  const handleObjeto = {
    ...
    reenviar_post_calificacion: (event) => {
      setDatos({ ...datos, reenviar_post_calificacion: event.target.value });
    },
    porcentaje_en_total_curso: (event) => {
      setDatos({ ...datos, porcentaje_en_total_curso: event.target.value });
    },
    show: (event) => {
    ...
    get: async (event) => {
      ...
      try {
        ...
        const response = await fetch(
          `${urlBaseApi}/api/tarea/${id_tarea}/${id_curso}`,
          opciones
        );
      ...
      if (id_tarea != -1) {
        let resRowData = {
          ...
          fecha_hora_fin:
            ...
            ":00",
          reenviar_post_calificacion:
            datos.reenviar_post_calificacion.toString(),
          id_curso: id_curso,
          porcentaje_en_total_curso: datos.porcentaje_en_total_curso,
        };
        ...
        resData.append(
          "reenviar_post_calificacion",
          datos.reenviar_post_calificacion
        );
        resData.append(
          "porcentaje_en_total_curso",
          datos.porcentaje_en_total_curso
        );
        ...
        setMostrarSpinner(true);
        const response = await fetch(`${urlBaseApi}/api/tarea`, opciones);
        setMostrarSpinner(false);
        if (response.ok) {
          setPopup({
            mostrar: true,
            titulo: "Listo",
            contenido: "Tarea creada.",
            data_switch: "cerrar_ventana",
          });
          return;
        } else {
        ...
        >
          <div className="modal-content">
            <div className="modal-header border-bottom-gray">
              <div className="pr-2">
                <h5
                  ...
                  id="tagModalTitle"
                >
                  {id_tarea != -1 ? "Editar Tarea" : "Crear Tarea"}
                </h5>
              </div>
            </div>
            <div className="modal-body">
              ...
              <div className="col-lg-12">
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    onChange={handleObjeto.descripcion}
              ...
              <div className="col-lg-12">
                <div className="form-group">
                  <label className="label-text" style={{ display: "block" }}>
                    Fecha de inicio
                  </label>
                  <input
                  ...
                  <div
                    style={{
                      position: "absolute",
                      zIndex: "999",
                      backgroundColor: temaActual ? "#ffffff" : "#1B1B1B",
                  ...
                  {erroresCampos["fecha_hora_inicio"].length > 0 && (
                    <SpamError mensaje={erroresCampos["fecha_hora_inicio"]} />
                  )}
                </div>
              </div>
              <div className="col-lg-12">
                <div className="form-group">
                  ...
                  {erroresCampos["fecha_hora_fin"].length > 0 && (
                    <SpamError mensaje={erroresCampos["fecha_hora_fin"]} />
                  )}
                </div>
              </div>
              <div className="col-lg-12">
                <div className="form-group">
                  <label className="label-text">
                    Reenviar tarea post calificación
                  </label>
                  <select
                    onChange={handleObjeto.reenviar_post_calificacion}
                    style={{ height: "50px" }}
                    value={datos.reenviar_post_calificacion}
                    name="reenviar_post_calificacion"
                    className={`form-control ${
                      temaActual == 1 ? "" : "select-dark"
                    }`}
                  >
                  ...
                  {erroresCampos["reenviar_post_calificacion"].length > 0 && (
                    <SpamError
                      mensaje={erroresCampos["reenviar_post_calificacion"]}
                    />
                  )}
                </div>
              </div>
              <div className="col-lg-12">
                <div className="form-group">
                  ...
                  <select
                    onChange={handleObjeto.porcentaje_en_total_curso}
                    value={datos.porcentaje_en_total_curso}
                  ...
                  {erroresCampos["porcentaje_en_total_curso"].length > 0 && (
                    <SpamError
            ...
            <div className="modal-footer border-top-gray">
              <button
                ...
                onClick={handleObjeto.save}
              >
                {id_tarea != -1 ? "Guardar" : "Crear"}
              </button>
              <button
                type="button"
                className="btn theme-btn theme-btn-dark mb-2"
                onClick={handleObjeto.close}
              >
                {" "}
                Cancelar{" "}
              </button>
