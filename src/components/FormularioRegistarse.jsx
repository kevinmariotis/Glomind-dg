...
function FormularioRegistrarse() {
  ...
  const [popUpErrores, setPopupErrores] = useState({
    mostrar: false,
    titulo: "Corrige los campos",
    contenido:
      "Algunos campos contienen datos incorrectos o faltantes, por favor corrígelos.",
  });
  ...
  const handlePromociones = (event) => {
    setPromociones(event.target.checked);
  };
  const handleTerminosCondiciones = (event) => {
    setTerminosCondiciones(event.target.checked);
  };
    ...
    try {
      ...
      if (response.ok) {
        setPopup({
          ...
          titulo: "Listo!",
          contenido:
            "Haz creado tu usuario, te invitamos a iniciar sesión, al presionar aceptar te enviaremos a la página de inicio de sesión.",
        });
        return;
      } else {
        ...
        if (data.codigo != "") {
          const contenidos = {
            sesioniniciada: {
              ...
              contenido: "Ya tenias la sesión iniciada previamente",
            },
            suspendido: {
              titulo: "Suspendido",
              ...
              <div className="col-lg-7 mx-auto">
                <div className="card card-item card-round">
                  <div className="card-body p-0">
                    <h3
                      className="card-title text-center fs-40 lh-35 pb-2 mb-4"
                      style={{ color: "var(--Lavander)" }}
                    >
                      ¡Bienvenid@ a Glomind!
                    </h3>
                    <h3 className="card-title text-start fs-24 lh-35 pb-4">
                      Crea una cuenta
                    </h3>
                    <form method="post" className="pt-4">
                          ...
                          <div className="input-group-append">
                            <button
                              className={`btn theme-btn theme-btn-transparent toggle-password ${
                                contrasenaVisible ? "active" : ""
                              }`}
                              type="button"
                              onClick={handleToggleContrasena}
                            >
                              <svg
                                className="eye-on"
                      ...
                      <div className="input-box">
                        ...
                        {erroresCampos["g-recaptcha-response"].length > 0 && (
                          <SpamError
                            mensaje={erroresCampos["g-recaptcha-response"]}
                          />
                        )}
                      </div>
                      <div className="btn-box">
                        <div className="custom-control custom-checkbox mb-2 fs-15">
                          <input
                            ...
                            id="recibir_promociones"
                            name="recibir_promociones"
                          />
                          <label
                            className="custom-control-label custom--control-label lh-20"
                            htmlFor="recibir_promociones"
                          >
                            Sí! Quiero aprovechar al máximo la plataforma
                            recibiendo correos electrónicos con ofertas
                            exclusivas, recomendaciones personales y consejos de
                            aprendizaje!
                          </label>
                        </div>
                        <div className="custom-control custom-checkbox mb-4 fs-15">
                          ...
                          <br />
                          {erroresCampos["terminos_y_condiciones"].length >
                            0 && (
                            <SpamError
                              mensaje={erroresCampos["terminos_y_condiciones"]}
                            />
                          )}
                        </div>
                        <button
                          className="btn theme-btn btn-round w-100 mt-5"
                          type="submit"
                          disabled={botonRegistrarseEstado}
                          onClick={postData}
                        >
                          Registrar cuenta{" "}
