...
function FormularioPlayHeader({id_curso=-1, nombre_curso='', favorito=-1, archivado=-1, tiene_review=-1, porcentaje_progreso=-1, instructor_edita_contenido=false, curso_url_amigable=null, es_docente=false, callBackFavoritoCambiado=()=>{}, }) {        
        ...
        if(favorito==0){
            ...
            try {                
                ...
                if (response.ok) {                    
                    ...
                    mensajesDeError(setPopup, response.status, (typeof data.datos !== 'undefined') ? data.datos : {}, false, {'titulo': '', 'contenido': ''});
                }                
            }catch (error) {
                console.error('Error de conexión:', error);
            }
            ...
            try {                
                const response = await fetch(`${urlBaseApi}/api/cursofavorito/${id_curso}`, opciones);
                const data = await response.json();            
                if (response.ok) {                    
                    callBackFavoritoCambiado();
                    return;
    ...
    const establecerArchivarCurso = async (event) => {        
        ...
        try {                
            const response = await fetch(`${urlBaseApi}/api/cursomatriculacion/archivar/${id_curso}`, opciones);                
            const data = await response.json();            
            if (response.ok) {                    
                callBackFavoritoCambiado();
                return;
                ...
                <div className="modal-dialog modal-dialog-centered" role="document">
                    <div className="modal-content">
                        <div className="modal-header border-bottom-gray">
                            <div className="pr-2">
                                <h5 className="modal-title fs-19 font-weight-semi-bold lh-24" id="ratingModalTitle">
                                    Cómo calificaría este curso?
                                </h5>
                            </div>
                            <button onClick={()=>{ setMostrarModalRating(!mostrarModalRating); }} type="button" className="close" data-dismiss="modal" aria-label="Close">
                        ...
                        <div className="modal-body text-center py-5">                             
                            {tiene_review==0 ?                        
                                ...
                                <div className="input-box col-lg-12" style={{marginTop:'20px'}}>
                                    <label className="label-text">Deja tu reseña</label>
                                    <div className="form-group">
                                        <textarea className="form-control form--control pl-3" name="message" onKeyUp={handleMensajeResena} placeholder="Escribe el mensaje" maxLength="2048" rows="5"></textarea>                                                                        
                                        {erroresCampos['comentario'].length > 0 && (<SpamError mensaje={erroresCampos['comentario']} />)}
                                    </div>
                                </div>
                                <div className="btn-box col-lg-12">                                                                        
                                    <button className="btn theme-btn" type="submit" onClick={enviarResenaSevidor}>Enviar reseña</button>
                                </div>
                                </> 
                    ...
                    <div className="container-fluid">
                        <div className="main-menu-content d-flex align-items-center">
                            ...
                            <div className="logo-box">
                                <Link to="/" className="logo"><img src={`${urlBase}/images/logo_principal_blanco.png`} alt="logo" /></Link>
                            </div>   
