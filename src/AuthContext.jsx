...
export const AuthProvider = ({ children }) => {
  ...
  const [temaActual, setTemaActual] = useState(
    sessionStorage.getItem("temaActual") ?? 1
  ...
  const login = ({ jwt, permisos, es_docente }) => {
    ...
    const permisos_array =
      permisos != "" ? JSON.parse(permisos) : Array(150).fill(0);
  ...
  useEffect(() => {
    //El objetivo de este effect es establecer si se esta autenticado, recobrar los permisos y establecer el jwt en el contexto cuando el sitio se ejecute por primera vez.
        ...
        if (jwt) {
          ...
          if (isExpired) {
            ...
            const response = await fetch(
              `${urlBaseApi}/api/sesion/validarToken`,
              opciones
            );
